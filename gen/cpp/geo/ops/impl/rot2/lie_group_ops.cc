...
Rot2<Scalar> LieGroupOps<Scalar>::FromTangent(const Eigen::Matrix<Scalar, 1, 1>& vec, const Scalar epsilon) {
    // Input arrays
...
template <typename Scalar>
Eigen::Matrix<Scalar, 1, 1> LieGroupOps<Scalar>::ToTangent(const Rot2<Scalar>& a, const Scalar epsilon) {
    // Input arrays
    ...
    // Output terms (1)
    res[0] = std::atan2(_a[1], _a[0] + epsilon*((((_a[0]) > 0) - ((_a[0]) < 0)) + 0.5));
...
Eigen::Matrix<Scalar, 1, 1> LieGroupOps<Scalar>::LocalCoordinates(const Rot2<Scalar>& a, const Rot2<Scalar>& b, const Scalar epsilon) {
    ...
    const Scalar _tmp3 = _a[0]*_tmp2 + _a[1]*_tmp1;

    // Output terms (1)
    res[0] = std::atan2(_a[0]*_tmp1 - _a[1]*_tmp2, _tmp3 + epsilon*((((_tmp3) > 0) - ((_tmp3) < 0)) + 0.5));
