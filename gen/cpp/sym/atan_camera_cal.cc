...
Eigen::Matrix<Scalar, 2, 1> ATANCameraCal<Scalar>::PixelFromCameraPoint(
  ...
  const Scalar _tmp2 = std::sqrt(_tmp1 * (point(0, 0) * point(0, 0)) +
                                 _tmp1 * (point(1, 0) * point(1, 0)) + epsilon);
...
Eigen::Matrix<Scalar, 3, 1> ATANCameraCal<Scalar>::CameraRayFromPixel(
  ...
  const Scalar _tmp2 = std::sqrt(epsilon + (_tmp1 * _tmp1) / (_self[1] * _self[1]) +
                                 (_tmp0 * _tmp0) / (_self[0] * _self[0]));
