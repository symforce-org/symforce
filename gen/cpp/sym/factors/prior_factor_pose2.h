...
void PriorFactorPose2(const sym::Pose2<Scalar>& value, const sym::Pose2<Scalar>& prior,
  ...
  const Scalar _tmp7 = _tmp0 * sqrt_info(0, 0) + _tmp1 * sqrt_info(0, 1) + _tmp6 * sqrt_info(0, 2);
  const Scalar _tmp8 = _tmp0 * sqrt_info(1, 0) + _tmp1 * sqrt_info(1, 1) + _tmp6 * sqrt_info(1, 2);
  const Scalar _tmp9 = _tmp0 * sqrt_info(2, 0) + _tmp1 * sqrt_info(2, 1) + _tmp6 * sqrt_info(2, 2);
  const Scalar _tmp10 = std::pow(_tmp5, Scalar(2));
  const Scalar _tmp11 = _tmp10 + std::pow(_tmp4, Scalar(2));
  const Scalar _tmp12 = 1 - _tmp4 * (-_tmp2 + _tmp3) / _tmp10;
  const Scalar _tmp13 = _tmp10 * _tmp12 / _tmp11;
  const Scalar _tmp14 = _tmp13 * sqrt_info(0, 2);
  const Scalar _tmp15 = _tmp13 * sqrt_info(1, 2);
  const Scalar _tmp16 = _tmp13 * sqrt_info(2, 2);
  const Scalar _tmp17 =
      std::pow(_tmp12, Scalar(2)) * std::pow(_tmp5, Scalar(4)) / std::pow(_tmp11, Scalar(2));
  ...
  if (hessian != nullptr) {
    ...
    _hessian(1, 2) = 0;
    _hessian(2, 0) = _tmp14 * sqrt_info(0, 0) + _tmp15 * sqrt_info(1, 0) + _tmp16 * sqrt_info(2, 0);
    _hessian(2, 1) = _tmp14 * sqrt_info(0, 1) + _tmp15 * sqrt_info(1, 1) + _tmp16 * sqrt_info(2, 1);
    _hessian(2, 2) = _tmp17 * std::pow(sqrt_info(0, 2), Scalar(2)) +
                     _tmp17 * std::pow(sqrt_info(1, 2), Scalar(2)) +
                     _tmp17 * std::pow(sqrt_info(2, 2), Scalar(2));
  }

  if (rhs != nullptr) {
    Eigen::Matrix<Scalar, 3, 1>& _rhs = (*rhs);

    _rhs(0, 0) = _tmp7 * sqrt_info(0, 0) + _tmp8 * sqrt_info(1, 0) + _tmp9 * sqrt_info(2, 0);
    _rhs(1, 0) = _tmp7 * sqrt_info(0, 1) + _tmp8 * sqrt_info(1, 1) + _tmp9 * sqrt_info(2, 1);
    _rhs(2, 0) = _tmp14 * _tmp7 + _tmp15 * _tmp8 + _tmp16 * _tmp9;
  }
