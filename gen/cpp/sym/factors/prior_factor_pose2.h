...
void PriorFactorPose2(const sym::Pose2<Scalar>& value, const sym::Pose2<Scalar>& prior,
  ...
  const Scalar _tmp7 = _tmp0 * sqrt_info(0, 1) + _tmp1 * sqrt_info(0, 2) +
                       sqrt_info(0, 0) * std::atan2(_tmp4, _tmp6);
  const Scalar _tmp8 = _tmp0 * sqrt_info(1, 1) + _tmp1 * sqrt_info(1, 2);
  const Scalar _tmp9 = std::pow(_tmp6, Scalar(2));
  const Scalar _tmp10 = -_tmp4 * (-_tmp2 + _tmp3) / _tmp9 + _tmp5 / _tmp6;
  const Scalar _tmp11 = std::pow(_tmp4, Scalar(2)) + _tmp9;
  const Scalar _tmp12 = _tmp10 * _tmp9 * sqrt_info(0, 0) / _tmp11;
  const Scalar _tmp13 = std::pow(sqrt_info(2, 2), Scalar(2));
  ...
  if (hessian != nullptr) {
    ...
    _hessian(0, 0) = std::pow(_tmp10, Scalar(2)) * std::pow(_tmp6, Scalar(4)) *
                     std::pow(sqrt_info(0, 0), Scalar(2)) / std::pow(_tmp11, Scalar(2));
    _hessian(1, 0) = _tmp12 * sqrt_info(0, 1);
    _hessian(2, 0) = _tmp12 * sqrt_info(0, 2);
    _hessian(0, 1) = 0;
    ...
    _hessian(1, 2) = 0;
    _hessian(2, 2) =
        _tmp13 + std::pow(sqrt_info(0, 2), Scalar(2)) + std::pow(sqrt_info(1, 2), Scalar(2));
  }

  if (rhs != nullptr) {
    Eigen::Matrix<Scalar, 3, 1>& _rhs = (*rhs);

    _rhs(0, 0) = _tmp12 * _tmp7;
    _rhs(1, 0) = _tmp7 * sqrt_info(0, 1) + _tmp8 * sqrt_info(1, 1);
    _rhs(2, 0) = _tmp1 * _tmp13 + _tmp7 * sqrt_info(0, 2) + _tmp8 * sqrt_info(1, 2);
  }
