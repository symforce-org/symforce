...
void SphericalReprojectionDelta(const sym::Pose3<Scalar>& source_pose,
  ...
  const Scalar _tmp0 = -2 * std::pow(_target_pose[1], Scalar(2));
  const Scalar _tmp1 = 1 - 2 * std::pow(_target_pose[2], Scalar(2));
  const Scalar _tmp2 = 2 * _source_pose[1];
  const Scalar _tmp3 = _source_pose[3] * _tmp2;
  const Scalar _tmp4 = 2 * _source_pose[0] * _source_pose[2];
  const Scalar _tmp5 = std::pow(Scalar(epsilon + std::pow(p_camera_source(0, 0), Scalar(2)) +
                                       std::pow(p_camera_source(1, 0), Scalar(2)) +
                                       std::pow(p_camera_source(2, 0), Scalar(2))),
                                Scalar(Scalar(-1) / Scalar(2)));
  const Scalar _tmp6 = _tmp5 * p_camera_source(2, 0);
  const Scalar _tmp7 = _source_pose[0] * _tmp2;
  const Scalar _tmp8 = 2 * _source_pose[3];
  const Scalar _tmp9 = _source_pose[2] * _tmp8;
  const Scalar _tmp10 = _tmp5 * p_camera_source(1, 0);
  const Scalar _tmp11 = -2 * std::pow(_source_pose[2], Scalar(2));
  const Scalar _tmp12 = 1 - 2 * std::pow(_source_pose[1], Scalar(2));
  const Scalar _tmp13 = _tmp5 * p_camera_source(0, 0);
  const Scalar _tmp14 = _tmp10 * (_tmp7 - _tmp9) + _tmp13 * (_tmp11 + _tmp12) +
                        _tmp6 * (_tmp3 + _tmp4) +
                        source_inverse_range * (_source_pose[4] - _target_pose[4]);
  const Scalar _tmp15 = 2 * _target_pose[3];
  const Scalar _tmp16 = _target_pose[2] * _tmp15;
  const Scalar _tmp17 = 2 * _target_pose[0] * _target_pose[1];
  const Scalar _tmp18 = _source_pose[2] * _tmp2;
  const Scalar _tmp19 = _source_pose[0] * _tmp8;
  const Scalar _tmp20 = -2 * std::pow(_source_pose[0], Scalar(2));
  const Scalar _tmp21 = _tmp10 * (_tmp11 + _tmp20 + 1) + _tmp13 * (_tmp7 + _tmp9) +
                        _tmp6 * (_tmp18 - _tmp19) +
                        source_inverse_range * (_source_pose[5] - _target_pose[5]);
  const Scalar _tmp22 = 2 * _target_pose[2];
  const Scalar _tmp23 = _target_pose[0] * _tmp22;
  const Scalar _tmp24 = _target_pose[1] * _tmp15;
  const Scalar _tmp25 = _tmp10 * (_tmp18 + _tmp19) + _tmp13 * (-_tmp3 + _tmp4) +
                        _tmp6 * (_tmp12 + _tmp20) +
                        source_inverse_range * (_source_pose[6] - _target_pose[6]);
  const Scalar _tmp26 =
      _tmp14 * (_tmp0 + _tmp1) + _tmp21 * (_tmp16 + _tmp17) + _tmp25 * (_tmp23 - _tmp24);
  const Scalar _tmp27 = -2 * std::pow(_target_pose[0], Scalar(2));
  const Scalar _tmp28 = _target_pose[1] * _tmp22;
  const Scalar _tmp29 = _target_pose[0] * _tmp15;
  const Scalar _tmp30 =
      _tmp14 * (-_tmp16 + _tmp17) + _tmp21 * (_tmp1 + _tmp27) + _tmp25 * (_tmp28 + _tmp29);
  const Scalar _tmp31 =
      std::sqrt(std::pow(_tmp26, Scalar(2)) + std::pow(_tmp30, Scalar(2)) + epsilon);
  const Scalar _tmp32 = std::atan2(_tmp31, _tmp14 * (_tmp23 + _tmp24) + _tmp21 * (_tmp28 - _tmp29) +
                                               _tmp25 * (_tmp0 + _tmp27 + 1));
  const Scalar _tmp33 = std::min<Scalar>(_tmp32, -epsilon + target_calibration_storage(4, 0));
  const Scalar _tmp34 = (std::pow(_tmp33, Scalar(9)) * target_calibration_storage(8, 0) +
                         std::pow(_tmp33, Scalar(7)) * target_calibration_storage(7, 0) +
                         std::pow(_tmp33, Scalar(5)) * target_calibration_storage(6, 0) +
                         [&]() {
  ...
  if (reprojection_delta != nullptr) {
    ...
    _reprojection_delta(0, 0) = _tmp26 * _tmp34 * target_calibration_storage(0, 0) +
                                target_calibration_storage(2, 0) - target_pixel(0, 0);
    _reprojection_delta(1, 0) = _tmp30 * _tmp34 * target_calibration_storage(1, 0) +
  ...
  if (is_valid != nullptr) {
    ...
    _is_valid = std::max<Scalar>(0, (((-_tmp32 + target_calibration_storage(4, 0)) > 0) -
                                     ((-_tmp32 + target_calibration_storage(4, 0)) < 0)));
  }
