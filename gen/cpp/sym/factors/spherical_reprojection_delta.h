...
void SphericalReprojectionDelta(const sym::Pose3<Scalar>& source_pose,
  ...
  const Scalar _tmp3 = 2 * _source_pose[0];
  ...
  const Scalar _tmp9 = -2 * std::pow(_source_pose[2], Scalar(2));
  const Scalar _tmp10 = -2 * std::pow(_source_pose[0], Scalar(2));
  const Scalar _tmp11 = _tmp7 * p_camera_source(1, 0);
  const Scalar _tmp12 = _source_pose[0] * _tmp5;
  const Scalar _tmp13 = 2 * _source_pose[1] * _source_pose[2];
  const Scalar _tmp14 = _tmp7 * p_camera_source(2, 0);
  const Scalar _tmp15 = _tmp11 * (_tmp10 + _tmp9 + 1) + _tmp14 * (-_tmp12 + _tmp13) +
                        _tmp8 * (_tmp4 + _tmp6) +
                        source_inverse_range * (_source_pose[5] - _target_pose[5]);
  const Scalar _tmp16 = 2 * _target_pose[2];
  const Scalar _tmp17 = _target_pose[0] * _tmp16;
  const Scalar _tmp18 = _target_pose[1] * _tmp0;
  ...
  const Scalar _tmp22 = _tmp11 * (_tmp12 + _tmp13) + _tmp14 * (_tmp10 + _tmp19) +
                        _tmp8 * (_tmp20 - _tmp21) +
  ...
  const Scalar _tmp25 = _tmp11 * (_tmp4 - _tmp6) + _tmp14 * (_tmp20 + _tmp21) +
                        _tmp8 * (_tmp19 + _tmp9) +
                        source_inverse_range * (_source_pose[4] - _target_pose[4]);
  const Scalar _tmp26 =
      _tmp15 * (_tmp1 + _tmp2) + _tmp22 * (_tmp17 - _tmp18) + _tmp25 * (_tmp23 + _tmp24);
  const Scalar _tmp27 = std::pow(_tmp26, Scalar(2));
  const Scalar _tmp28 = -2 * std::pow(_target_pose[0], Scalar(2));
  const Scalar _tmp29 = _target_pose[1] * _tmp16;
  const Scalar _tmp30 = _target_pose[0] * _tmp0;
  const Scalar _tmp31 =
      _tmp15 * (_tmp23 + _tmp28 + 1) + _tmp22 * (_tmp29 + _tmp30) + _tmp25 * (-_tmp1 + _tmp2);
  const Scalar _tmp32 = std::pow(_tmp31, Scalar(2));
  const Scalar _tmp33 = _tmp27 + _tmp32 + epsilon;
  const Scalar _tmp34 = std::sqrt(_tmp33);
  const Scalar _tmp35 =
      _tmp15 * (_tmp29 - _tmp30) + _tmp22 * (_tmp24 + _tmp28) + _tmp25 * (_tmp17 + _tmp18);
  const Scalar _tmp36 = std::atan2(_tmp34, _tmp35);
  const Scalar _tmp37 = std::min<Scalar>(_tmp36, _target_calibration[4] - epsilon);
  const Scalar _tmp38 = _target_calibration[5] *
                            [&]() {
                              const Scalar base = _tmp37;
                              return base * base * base;
                            }() +
                        _target_calibration[6] * std::pow(_tmp37, Scalar(5)) +
                        _target_calibration[7] * std::pow(_tmp37, Scalar(7)) +
                        _target_calibration[8] * std::pow(_tmp37, Scalar(9)) + _tmp37;
  const Scalar _tmp39 = std::pow(_tmp38, Scalar(2)) / _tmp33;
  const Scalar _tmp40 = _target_calibration[9] * _tmp39;
  const Scalar _tmp41 = _target_calibration[10] * _tmp39;
  const Scalar _tmp42 = 2 * _tmp26 * _tmp31;
  const Scalar _tmp43 = _tmp38 / _tmp34;

  // Output terms (2)
  if (reprojection_delta != nullptr) {
    Eigen::Matrix<Scalar, 2, 1>& _reprojection_delta = (*reprojection_delta);

    _reprojection_delta(0, 0) = _target_calibration[0] * (_tmp26 * _tmp43 + 3 * _tmp27 * _tmp40 +
                                                          _tmp32 * _tmp40 + _tmp41 * _tmp42) +
                                _target_calibration[2] - target_pixel(0, 0);
    _reprojection_delta(1, 0) = _target_calibration[1] * (_tmp27 * _tmp41 + _tmp31 * _tmp43 +
                                                          3 * _tmp32 * _tmp41 + _tmp40 * _tmp42) +
  ...
  if (is_valid != nullptr) {
    ...
    _is_valid = std::max<Scalar>(
        0, (((_target_calibration[4] - _tmp36) > 0) - ((_target_calibration[4] - _tmp36) < 0)));
  }
