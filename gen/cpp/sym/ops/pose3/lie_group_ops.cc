...
sym::Pose3<Scalar> LieGroupOps<Pose3<Scalar>>::Interpolate(const sym::Pose3<Scalar>& a,
  ...
  const Scalar _tmp7 = 4 * std::pow(_tmp3, Scalar(2)) * std::pow(_tmp5, Scalar(2)) *
                       std::pow(alpha, Scalar(2)) / _tmp6;
  const Scalar _tmp8 = _a[0] * _b[2] - _a[1] * _b[3] - _a[2] * _b[0] + _a[3] * _b[1];
  const Scalar _tmp9 = -_a[0] * _b[1] + _a[1] * _b[0] - _a[2] * _b[3] + _a[3] * _b[2];
  const Scalar _tmp10 =
      std::sqrt(Scalar(std::pow(_tmp0, Scalar(2)) * _tmp7 + _tmp7 * std::pow(_tmp8, Scalar(2)) +
                       _tmp7 * std::pow(_tmp9, Scalar(2)) + std::pow(epsilon, Scalar(2))));
  const Scalar _tmp11 = (Scalar(1) / Scalar(2)) * _tmp10;
  const Scalar _tmp12 = std::cos(_tmp11);
  const Scalar _tmp13 = 2 * _tmp3 * _tmp5 * alpha * std::sin(_tmp11) / (_tmp10 * std::sqrt(_tmp6));
  const Scalar _tmp14 = _tmp0 * _tmp13;
  const Scalar _tmp15 = _tmp13 * _tmp9;
  const Scalar _tmp16 = _tmp13 * _tmp8;

  // Output terms (1)
  Eigen::Matrix<Scalar, 7, 1> _res;

  _res[0] = _a[0] * _tmp12 + _a[1] * _tmp15 - _a[2] * _tmp16 + _a[3] * _tmp14;
  _res[1] = -_a[0] * _tmp15 + _a[1] * _tmp12 + _a[2] * _tmp14 + _a[3] * _tmp16;
  _res[2] = _a[0] * _tmp16 - _a[1] * _tmp14 + _a[2] * _tmp12 + _a[3] * _tmp15;
  _res[3] = -_a[0] * _tmp14 - _a[1] * _tmp16 - _a[2] * _tmp15 + _a[3] * _tmp12;
