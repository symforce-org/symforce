...
Eigen::Matrix<Scalar, 2, 1> PolynomialCameraCal<Scalar>::PixelFromCameraPointWithJacobians(
  ...
  const Scalar _tmp16 = _tmp14 * point(1, 0);
  const Scalar _tmp17 = (((point(2, 0)) > 0) - ((point(2, 0)) < 0));
  const Scalar _tmp18 = _self[0] * point(0, 0);
  const Scalar _tmp19 =
      _tmp13 * (Scalar(1.0) * _tmp4 + Scalar(1.0) * _tmp6 + Scalar(1.0) * epsilon);
  const Scalar _tmp20 = _tmp13 * _tmp18;
  ...
  const Scalar _tmp29 = _tmp23 * _tmp24 + _tmp23 * _tmp26 + _tmp23 * _tmp28;
  ...
  const Scalar _tmp36 =
      Scalar(2.0) * _tmp25 * _tmp35 + Scalar(3.0) * _tmp27 * _tmp35 + _tmp35 * _tmp8;

  // Output terms (4)
  Eigen::Matrix<Scalar, 2, 1> _pixel;

  _pixel(0, 0) = _self[0] * _tmp15 + _self[2];
  _pixel(1, 0) = _self[1] * _tmp16 + _self[3];
  ...
  if (pixel_D_cal != nullptr) {
    ...
    _pixel_D_cal(0, 1) = 0;
    _pixel_D_cal(0, 2) = Scalar(1.0);
    _pixel_D_cal(0, 3) = 0;
    _pixel_D_cal(0, 4) = _tmp18 * _tmp19;
    _pixel_D_cal(0, 5) = _tmp11 * _tmp20;
    _pixel_D_cal(0, 6) = _tmp20 * _tmp9;
    _pixel_D_cal(1, 0) = 0;
    _pixel_D_cal(1, 1) = _tmp16;
    _pixel_D_cal(1, 2) = 0;
    _pixel_D_cal(1, 3) = Scalar(1.0);
    _pixel_D_cal(1, 4) = _tmp19 * _tmp21;
  ...
  if (pixel_D_point != nullptr) {
    ...
    _pixel_D_point(0, 0) = _self[0] * _tmp14 + _tmp20 * _tmp29;
    _pixel_D_point(0, 1) = _tmp18 * _tmp31;
    _pixel_D_point(0, 2) = -_tmp18 * _tmp33 + _tmp20 * _tmp36;
    _pixel_D_point(1, 0) = _tmp22 * _tmp29;
    _pixel_D_point(1, 1) = _self[1] * _tmp14 + _tmp21 * _tmp31;
    _pixel_D_point(1, 2) = -_tmp21 * _tmp33 + _tmp22 * _tmp36;
  }
