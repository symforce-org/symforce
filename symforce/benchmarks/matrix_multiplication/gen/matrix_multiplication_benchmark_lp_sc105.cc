...
__attribute__((noinline)) Eigen::Matrix<Scalar, 163, 163> ComputeDenseFixedLpSc105(
  ...
  Eigen::Matrix<Scalar, 105, 163> A = ComputeADenseLpSc105<Scalar>(x0, x1);
  Eigen::Matrix<Scalar, 105, 163> B = ComputeBDenseLpSc105<Scalar>(x0, x1);
  return A.transpose() * B;
}

template <typename Scalar>
__attribute__((noinline)) Eigen::Matrix<Scalar, Eigen::Dynamic, Eigen::Dynamic>
ComputeDenseDynamicLpSc105(const Scalar x0, const Scalar x1) {
  const auto A = ComputeADenseDynamicLpSc105<Scalar>(x0, x1);
  const auto B = ComputeBDenseDynamicLpSc105<Scalar>(x0, x1);
  return A.transpose() * B;
}

template <typename Scalar>
__attribute__((noinline)) Eigen::SparseMatrix<Scalar> ComputeSparseLpSc105(const Scalar x0,
                                                                           const Scalar x1) {
  Eigen::SparseMatrix<Scalar> A = ComputeALpSc105<Scalar>(x0, x1);
  Eigen::SparseMatrix<Scalar> B = ComputeBLpSc105<Scalar>(x0, x1);
  return A.transpose() * B;
}
  ...
  Scalar sum = 0.0;
  {
    ...
    for (Scalar x0 = 0.1; x0 < 100.0; x0 += 0.1) {
      for (Scalar x1 = 0.1; x1 < 100.0; x1 += 0.1) {
        auto mat = ComputeSparseLpSc105(x0, x1);
        sum += mat.valuePtr()[0];
      }
    }
  }
...
TEMPLATE_TEST_CASE("dense_dynamic_lp_sc105", "", double, float) {
  ...
  Scalar sum = 0.0;
  {
    ...
    for (Scalar x0 = 0.1; x0 < 100.0; x0 += 0.1) {
      for (Scalar x1 = 0.1; x1 < 100.0; x1 += 0.1) {
        auto mat = ComputeDenseDynamicLpSc105<Scalar>(x0, x1);
        sum += mat(0, 0);
      }
    }
  }
...
TEMPLATE_TEST_CASE("flattened_lp_sc105", "", double, float) {
  ...
  Scalar sum = 0.0;
  {
    ...
    for (Scalar x0 = 0.1; x0 < 100.0; x0 += 0.1) {
      for (Scalar x1 = 0.1; x1 < 100.0; x1 += 0.1) {
        auto mat = ComputeAtBLpSc105(x0, x1);
        sum += mat.valuePtr()[0];
      }
    }
  }
