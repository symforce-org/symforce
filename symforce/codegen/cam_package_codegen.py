...
def generate(config: CodegenConfig, output_dir: str = None) -> str:
    ...
    if isinstance(config, PythonConfig):
        ...
        for name in ("cam_package_python_test.py",):
            templates.add(
                template_path=pathlib.Path(template_dir, "tests", name + ".jinja"),
        ...
        templates.add(template_dir / "camera.h.jinja", cam_package_dir / "camera.h", camera_data())
        templates.add(
            template_dir / "posed_camera.h.jinja",
            cam_package_dir / "posed_camera.h",
            posed_camera_data(),
        )
        ...
        for name in (
            ...
            templates.add(
                pathlib.Path(template_dir, "..", "tests", name + ".jinja"),
                pathlib.Path(output_dir, "tests", name),
                dict(
                    Codegen.common_data(),
                    all_types=DEFAULT_CAM_TYPES,
                    cpp_cam_types=[
