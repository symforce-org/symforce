...
def generate_matching_residual_code(output_dir: Path) -> None:
    ...
    codegen_with_linearization.generate_function(output_dir=output_dir)
...
def generate(output_dir: Path) -> None:
    ...
    values_codegen.generate_values_keys(build_values(NUM_POSES)[0], output_dir)
    build_codegen_object(NUM_POSES).generate_function(output_dir, namespace="robot_3d_localization")

    values = build_values(NUM_POSES)[0]
    template_util.render_template(
        template_path=os.fspath(Path(__file__).parent / "measurements.cc.jinja"),
        data=dict(
        ...
        output_path=os.fspath(output_dir / "measurements.cc"),
        template_dir=os.fspath(Path(__file__).parent),
    )

    template_util.render_template(
        template_path=os.fspath(Path(__file__).parent / "measurements.h.jinja"),
        data={},
        output_path=os.fspath(output_dir / "measurements.h"),
        template_dir=os.fspath(Path(__file__).parent),
    )
