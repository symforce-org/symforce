...
from symforce.expressions.op_probabilities import OpProbability
from symforce.expressions.op_probabilities import DEFAULT_UNARY_OPS
from symforce.expressions.op_probabilities import DEFAULT_BINARY_OPS
from symforce.expressions.op_probabilities import DEFAULT_LEAVES
...
class UnaryBinaryExpressionGen:
    ...
    def generate_D(
        ...
        D: T.List[T.Union[T.List[float], np.ndarray]]

        try:
            import numba

            D = []
            D.append(np.array([0] + ([num_leaves ** e for e in range(1, 2 * max_ops + 1)])))
            ...
            for n in range(1, max_ops + 1):  # number of operators
                D.append(next_row(num_leaves, max_ops, n, D[-1], p1, p2))
        except ImportError:
            D = []
            D.append([0.0] + ([num_leaves ** e for e in range(1, 2 * max_ops + 1)]))

            for n in range(1, max_ops + 1):
                s = [0.0]
                for e in range(1, 2 * max_ops - n + 1):  # number of empty nodes
                    s.append(num_leaves * s[e - 1] + p1 * D[n - 1][e] + p2 * D[n - 1][e + 1])
                D.append(s)
