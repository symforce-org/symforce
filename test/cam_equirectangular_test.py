...
class EquirectangularTest(LieGroupOpsTestMixin, CamTestMixin, TestCase):
    """
    ...
    @classmethod
    def element(cls) -> cam.EquirectangularCameraCal:
        [f_x, f_y, c_x, c_y] = np.random.uniform(low=0.0, high=1000.0, size=(4,))
        return cam.EquirectangularCameraCal(focal_length=(f_x, f_y), principal_point=(c_x, c_y))
    ...
    def test_invalid_pixels(self) -> None:
        ...
        f_x, f_y = (380, 380)
        c_x, c_y = (320, 240)
        cam_cal = cam.EquirectangularCameraCal(focal_length=(f_x, f_y), principal_point=(c_x, c_y))
        invalid_pixels = [
            geo.V2(f_x * (np.pi + 1e-6) + c_x, c_y),
            geo.V2(c_x, f_y * (np.pi / 2.0 + 1e-6) + c_y),
            geo.V2(f_x * (-np.pi - 1e-6) + c_x, c_y),
            geo.V2(c_x, f_y * (-np.pi / 2.0 - 1e-6) + c_y),
            geo.V2(1000, 1000),
            geo.V2(-1000, -1000),
