...
class CamOrthographicTest(LieGroupOpsTestMixin, CamCalTestMixin, TestCase):
    """
    Test the OrthographicCameraCal class.
    Note the mixin that tests all storage ops and camera projection/reprojection ops.
    """

    @classmethod
    def element(cls) -> sf.OrthographicCameraCal:
        [f_x, f_y, c_x, c_y] = np.random.uniform(low=0.0, high=1000.0, size=(4,))
        return sf.OrthographicCameraCal(focal_length=(f_x, f_y), principal_point=(c_x, c_y))
