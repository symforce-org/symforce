...
TEST_CASE("Verify handwritten and auto-derivative impls are equivalent", "[slam]") {
  ...
  for (int i_ = 0; i_ < 10; i_++) {
    ...
    CHECK(sym::IsClose(new_covariance, new_covariance_auto, 1e-14));
    CHECK(sym::IsClose(new_DR_D_gyro_bias, new_DR_D_gyro_bias_auto, 1e-14));
    CHECK(sym::IsClose(new_Dv_D_accel_bias, new_Dv_D_accel_bias_auto, 1e-14));
    CHECK(sym::IsClose(new_Dv_D_gyro_bias, new_Dv_D_gyro_bias_auto, 1e-14));
    CHECK(sym::IsClose(new_Dp_D_accel_bias, new_Dp_D_accel_bias_auto, 1e-14));
    CHECK(sym::IsClose(new_Dp_D_gyro_bias, new_Dp_D_gyro_bias_auto, 1e-14));
  }
}
