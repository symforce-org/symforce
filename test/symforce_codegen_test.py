...
class SymforceCodegenTest(TestCase):
    ...
    def test_return_geo_type_from_generated_python_function(self) -> None:
        ...
        with the default namespace can be called.
        ...
        co = codegen.Codegen.function(func=identity, config=codegen.PythonConfig())
        codegen_data = co.generate_function(output_dir=output_dir)

        # NOTE(brad): Currently, the function directory is named the namespace used. As
        # this may change in the future (which would break the test), I assert that it
        # is equal to the namespace field of the codegen object, which is also set to the
        # namespace of the object as a side effect of generate_function (this could also
        # change in the future). I assert they are equal to reduce the liklihood of the test
        # silently breaking. If this assertion fails, whoever fixes it will need to find a
        # new way to figure out the namespace of the generated function.
        namespace = codegen_data.function_dir.name
        assert namespace == co.namespace, "Test itself is broken, unsure of namespace name"
    ...
    def test_function_codegen_python(self) -> None:
        ...
        az_el_codegen_data = az_el_codegen.generate_function(output_dir, namespace="sym")
    ...
    def test_function_codegen_cpp(self) -> None:
        ...
        az_el_codegen_data = az_el_codegen.generate_function(output_dir=output_dir, namespace="sym")
    ...
    def test_with_jacobians(self) -> None:
        ...
        for codegen_function in codegens.values():
            codegen_function.generate_function(namespace="sym", output_dir=output_dir)
    ...
    def test_with_jacobians_values(self) -> None:
        ...
        codegen_with_jacobians.generate_function(
    ...
    def test_with_jacobians_multiple_outputs(self) -> None:
        ...
        for spec in specs:
            with self.subTest(spec=spec):
                ...
                curr_codegen.generate_function(namespace="sym", output_dir=output_dir)
    ...
    def test_function_with_dataclass(self) -> None:
        ...
        dataclass_t = codegen_util.load_generated_lcmtype(
            "gen", "dataclass_t", dataclass_codegen_data.python_types_dir
        )()
