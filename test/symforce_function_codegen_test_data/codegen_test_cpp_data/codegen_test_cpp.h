...
#include "./codegen_test_cpp/storage_ops.h"
...
void CodegenTestCpp(const Scalar x, const Scalar y, const geo::Rot3<Scalar>& rot, const codegen_test_cpp::constants_t& constants, const codegen_test_cpp::states_t& states, Scalar* const foo, Scalar* const bar) {
    // Input arrays
    const Eigen::Matrix<Scalar, 4, 1>& _rot = rot.Data();
    std::vector<Scalar> constants_array;
    codegen_test_cpp::StorageOps::ToStorage<codegen_test_cpp::constants_t>(constants, &constants_array);
    const Eigen::Matrix<Scalar, 1, 1> _constants(constants_array.data());
    std::vector<Scalar> states_array;
    codegen_test_cpp::StorageOps::ToStorage<codegen_test_cpp::states_t>(states, &states_array);
