...
void CrossAndDistanceJacobians(const Eigen::Matrix<Scalar, 3, 1>& a,
  ...
  if (distance != nullptr) {
    ...
    _distance =
        std::sqrt(epsilon + std::pow<Scalar>(_tmp0 + a(0, 0), 2) +
                  std::pow<Scalar>(_tmp1 + a(1, 0), 2) + std::pow<Scalar>(_tmp2 + a(2, 0), 2));
  }
