// -----------------------------------------------------------------------------
// This file was autogenerated by symforce. Do NOT modify by hand.
// -----------------------------------------------------------------------------
#pragma once

#include <Eigen/Dense>

namespace gnc_factors {

/**
 * This function was autogenerated from a symbolic function. Do not modify by hand.
 *
 * Symbolic function: barron_factor
 * Arg type(s): Matrix51, Matrix51, Symbol, Symbol
 * Return type(s): Matrix51
 *     geo.Matrix: Jacobian for args 0 (x)
 *     geo.Matrix: Hessian for args 0 (x)
 *     geo.Matrix: rhs for args 0 (x)
 */
template <typename Scalar>
void BarronResidual(const Eigen::Matrix<Scalar, 5, 1>& x, const Eigen::Matrix<Scalar, 5, 1>& y,
                    const Scalar mu, const Scalar eps,
                    Eigen::Matrix<Scalar, 5, 1>* const res = nullptr,
                    Eigen::Matrix<Scalar, 5, 5>* const jacobian = nullptr,
                    Eigen::Matrix<Scalar, 5, 5>* const hessian = nullptr,
                    Eigen::Matrix<Scalar, 5, 1>* const rhs = nullptr) {
  // Total ops: 123

  // Input arrays

  // Intermediate terms (45)
  const Scalar _tmp0 = -std::sqrt(eps);
  const Scalar _tmp1 = x(0, 0) - y(0, 0);
  const Scalar _tmp2 = std::pow<Scalar>(_tmp1, 2);
  const Scalar _tmp3 = 1.0 / (eps - mu + 1);
  const Scalar _tmp4 = eps + std::fabs(_tmp3);
  const Scalar _tmp5 = 1.0 / (_tmp4);
  const Scalar _tmp6 = _tmp2 * _tmp5 + 1;
  const Scalar _tmp7 = 2 - _tmp3;
  const Scalar _tmp8 = _tmp7 + eps * (2 * std::min<Scalar>(0, (((_tmp7) > 0) - ((_tmp7) < 0))) + 1);
  const Scalar _tmp9 = (1.0 / 2.0) * _tmp8;
  const Scalar _tmp10 = 2 * _tmp4 / _tmp8;
  const Scalar _tmp11 = _tmp10 * (std::pow<Scalar>(_tmp6, _tmp9) - 1) + eps;
  const Scalar _tmp12 = std::sqrt(_tmp11);
  const Scalar _tmp13 = _tmp0 + _tmp12;
  const Scalar _tmp14 = x(1, 0) - y(1, 0);
  const Scalar _tmp15 = std::pow<Scalar>(_tmp14, 2);
  const Scalar _tmp16 = _tmp15 * _tmp5 + 1;
  const Scalar _tmp17 = _tmp10 * (std::pow<Scalar>(_tmp16, _tmp9) - 1) + eps;
  const Scalar _tmp18 = std::sqrt(_tmp17);
  const Scalar _tmp19 = _tmp0 + _tmp18;
  const Scalar _tmp20 = x(2, 0) - y(2, 0);
  const Scalar _tmp21 = std::pow<Scalar>(_tmp20, 2);
  const Scalar _tmp22 = _tmp21 * _tmp5 + 1;
  const Scalar _tmp23 = _tmp10 * (std::pow<Scalar>(_tmp22, _tmp9) - 1) + eps;
  const Scalar _tmp24 = std::sqrt(_tmp23);
  const Scalar _tmp25 = _tmp0 + _tmp24;
  const Scalar _tmp26 = x(3, 0) - y(3, 0);
  const Scalar _tmp27 = std::pow<Scalar>(_tmp26, 2);
  const Scalar _tmp28 = _tmp27 * _tmp5 + 1;
  const Scalar _tmp29 = _tmp10 * (std::pow<Scalar>(_tmp28, _tmp9) - 1) + eps;
  const Scalar _tmp30 = std::sqrt(_tmp29);
  const Scalar _tmp31 = _tmp0 + _tmp30;
  const Scalar _tmp32 = x(4, 0) - y(4, 0);
  const Scalar _tmp33 = std::pow<Scalar>(_tmp32, 2);
  const Scalar _tmp34 = _tmp33 * _tmp5 + 1;
  const Scalar _tmp35 = _tmp10 * (std::pow<Scalar>(_tmp34, _tmp9) - 1) + eps;
  const Scalar _tmp36 = std::sqrt(_tmp35);
  const Scalar _tmp37 = _tmp0 + _tmp36;
  const Scalar _tmp38 = _tmp9 - 1;
  const Scalar _tmp39 = _tmp1 * std::pow<Scalar>(_tmp6, _tmp38) / _tmp12;
  const Scalar _tmp40 = _tmp14 * std::pow<Scalar>(_tmp16, _tmp38) / _tmp18;
  const Scalar _tmp41 = _tmp20 * std::pow<Scalar>(_tmp22, _tmp38) / _tmp24;
  const Scalar _tmp42 = _tmp26 * std::pow<Scalar>(_tmp28, _tmp38) / _tmp30;
  const Scalar _tmp43 = _tmp32 * std::pow<Scalar>(_tmp34, _tmp38) / _tmp36;
  const Scalar _tmp44 = _tmp8 - 2;

  // Output terms (4)
  if (res != nullptr) {
    Eigen::Matrix<Scalar, 5, 1>& _res = (*res);

    _res(0, 0) = _tmp13;
    _res(1, 0) = _tmp19;
    _res(2, 0) = _tmp25;
    _res(3, 0) = _tmp31;
    _res(4, 0) = _tmp37;
  }

  if (jacobian != nullptr) {
    Eigen::Matrix<Scalar, 5, 5>& _jacobian = (*jacobian);

    _jacobian(0, 0) = _tmp39;
    _jacobian(0, 1) = 0;
    _jacobian(0, 2) = 0;
    _jacobian(0, 3) = 0;
    _jacobian(0, 4) = 0;
    _jacobian(1, 0) = 0;
    _jacobian(1, 1) = _tmp40;
    _jacobian(1, 2) = 0;
    _jacobian(1, 3) = 0;
    _jacobian(1, 4) = 0;
    _jacobian(2, 0) = 0;
    _jacobian(2, 1) = 0;
    _jacobian(2, 2) = _tmp41;
    _jacobian(2, 3) = 0;
    _jacobian(2, 4) = 0;
    _jacobian(3, 0) = 0;
    _jacobian(3, 1) = 0;
    _jacobian(3, 2) = 0;
    _jacobian(3, 3) = _tmp42;
    _jacobian(3, 4) = 0;
    _jacobian(4, 0) = 0;
    _jacobian(4, 1) = 0;
    _jacobian(4, 2) = 0;
    _jacobian(4, 3) = 0;
    _jacobian(4, 4) = _tmp43;
  }

  if (hessian != nullptr) {
    Eigen::Matrix<Scalar, 5, 5>& _hessian = (*hessian);

    _hessian(0, 0) = _tmp2 * std::pow<Scalar>(_tmp6, _tmp44) / _tmp11;
    _hessian(0, 1) = 0;
    _hessian(0, 2) = 0;
    _hessian(0, 3) = 0;
    _hessian(0, 4) = 0;
    _hessian(1, 0) = 0;
    _hessian(1, 1) = _tmp15 * std::pow<Scalar>(_tmp16, _tmp44) / _tmp17;
    _hessian(1, 2) = 0;
    _hessian(1, 3) = 0;
    _hessian(1, 4) = 0;
    _hessian(2, 0) = 0;
    _hessian(2, 1) = 0;
    _hessian(2, 2) = _tmp21 * std::pow<Scalar>(_tmp22, _tmp44) / _tmp23;
    _hessian(2, 3) = 0;
    _hessian(2, 4) = 0;
    _hessian(3, 0) = 0;
    _hessian(3, 1) = 0;
    _hessian(3, 2) = 0;
    _hessian(3, 3) = _tmp27 * std::pow<Scalar>(_tmp28, _tmp44) / _tmp29;
    _hessian(3, 4) = 0;
    _hessian(4, 0) = 0;
    _hessian(4, 1) = 0;
    _hessian(4, 2) = 0;
    _hessian(4, 3) = 0;
    _hessian(4, 4) = _tmp33 * std::pow<Scalar>(_tmp34, _tmp44) / _tmp35;
  }

  if (rhs != nullptr) {
    Eigen::Matrix<Scalar, 5, 1>& _rhs = (*rhs);

    _rhs(0, 0) = _tmp13 * _tmp39;
    _rhs(1, 0) = _tmp19 * _tmp40;
    _rhs(2, 0) = _tmp25 * _tmp41;
    _rhs(3, 0) = _tmp31 * _tmp42;
    _rhs(4, 0) = _tmp37 * _tmp43;
  }
}  // NOLINT(readability/fn_size)

// NOLINTNEXTLINE(readability/fn_size)
}  // namespace gnc_factors
