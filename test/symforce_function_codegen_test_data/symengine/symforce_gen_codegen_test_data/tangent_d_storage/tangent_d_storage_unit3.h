...
Eigen::Matrix<Scalar, 2, 3> TangentDStorage(const sym::Unit3<Scalar>& a, const Scalar epsilon) {
  ...
  const Scalar _tmp8 = _tmp2 * _tmp4 * _tmp7;
  const Scalar _tmp9 = 2 / _tmp6;
  const Scalar _tmp10 = _a[1] * _tmp8;
  const Scalar _tmp11 = _a[1] * _tmp1 * _tmp4 * _tmp9;

  // Output terms (1)
  Eigen::Matrix<Scalar, 2, 3> _res;

  _res(0, 0) = _a[1] * _tmp2 * _tmp3 * _tmp7;
  _res(1, 0) = -_tmp3 * _tmp8;
  _res(0, 1) = -_tmp1 * (-_tmp0 * _tmp9 + 1) - _tmp2 * (-_tmp0 * _tmp7 + 1);
  _res(1, 1) = -_tmp10 - _tmp11;
  _res(0, 2) = _tmp10 + _tmp11;
  _res(1, 2) = _tmp1 * (-_tmp5 * _tmp9 + 1) + _tmp2 * (-_tmp5 * _tmp7 + 1);
