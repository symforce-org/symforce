...
class SymforceSimilarityIndexTest(TestCase):
    ...
    def test_from_codegen(self) -> None:
        ...
        with self.subTest(msg="different function names don't imply dissimilarity"):
            ...
            co_1 = codegen.Codegen.function(func=geo.Rot3.compose, name="some_name", config=config)
            co_2 = codegen.Codegen.function(
                func=geo.Rot3.compose, name="alternate_name", config=config
            )
        ...
        with self.subTest(msg="different doc-strings don't imply dissimilarity"):
            ...
            co_1 = codegen.Codegen.function(
                func=geo.Rot3.compose, docstring="a function", config=config
            )
            co_2 = codegen.Codegen.function(
                func=geo.Rot3.compose, docstring="multiply", config=config
            )

            self.assertEqual(SimilarityIndex.from_codegen(co_1), SimilarityIndex.from_codegen(co_2))

        with self.subTest(msg="Python functions are not similar to C++ functions"):
            co_cpp = codegen.Codegen.function(func=geo.Rot3.compose, config=codegen.CppConfig())
            co_python = codegen.Codegen.function(
                func=geo.Rot3.compose, config=codegen.PythonConfig()
            )
        ...
        with self.subTest(msg="Different configuration settings are dissimilar"):
            co_no_numba = codegen.Codegen.function(
                func=geo.Rot3.compose, config=codegen.PythonConfig(use_numba=False)
            )
            co_with_numba = codegen.Codegen.function(
                func=geo.Rot3.compose, config=codegen.PythonConfig(use_numba=True)
            )
        ...
        with self.subTest(msg="Different functions are dissimilar"):
            ...
            co_rot3 = codegen.Codegen.function(func=geo.Rot3.compose, config=config)
            co_rot2 = codegen.Codegen.function(func=geo.Rot2.compose, config=config)
        ...
        with self.subTest(msg="Sparse outputs must match to be similar"):
            inputs = Values(arg=geo.Rot3.symbolic("a"))
            outputs = Values(out=geo.Matrix(inputs["arg"].to_storage()))
