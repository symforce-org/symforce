...
#define INTERNAL_CATCH_CAPTURE(varName, macroName, ...)                               \
  Catch::Capturer varName(macroName, CATCH_INTERNAL_LINEINFO, Catch::ResultWas::Info, \
                          #__VA_ARGS__);                                              \
...
class ExeName : public Detail::ComposableParserImpl<ExeName> {
  ...
  template <typename LambdaT>
  static auto makeRef(LambdaT const& lambda) -> std::shared_ptr<Detail::BoundValueRefBase> {
...
struct Tag {
  Tag(StringRef original_, StringRef lowerCased_) : original(original_), lowerCased(lowerCased_) {}
...
class GeneratorWrapper final {
 ...
 public:
  ...
  GeneratorWrapper(GeneratorPtr<T> generator) : m_generator(std::move(generator)) {}
...
template <typename T>
std::enable_if_t<std::is_integral<T>::value && !std::is_same<T, bool>::value, GeneratorWrapper<T>>
random(T a, T b) {
  return GeneratorWrapper<T>(Catch::Detail::make_unique<RandomIntegerGenerator<T>>(a, b));
}

template <typename T>
std::enable_if_t<std::is_floating_point<T>::value, GeneratorWrapper<T>> random(T a, T b) {
  return GeneratorWrapper<T>(Catch::Detail::make_unique<RandomFloatingGenerator<T>>(a, b));
}
...
class Column {
 ...
 public:
  class iterator {
    ...
    iterator(Column const& column, EndTag) : m_column(column), m_pos(m_column.m_string.size()) {}
...
template <typename ArgT, typename MatcherT>
auto makeMatchExpr(ArgT&& arg, MatcherT const& matcher, StringRef const& matcherString)
...
class ContainsElementMatcher final : public MatcherGenericBase {
 ...
 public:
  ...
  ContainsElementMatcher(T2&& target, Equality2&& predicate)
      : m_desired(std::forward<T2>(target)), m_eq(std::forward<Equality2>(predicate)) {}
  ...
  bool match(RangeLike&& rng) const {
    ...
    return end(rng) != std::find_if(begin(rng), end(rng),
                                    [&](auto const& elem) { return m_eq(elem, m_desired); });
  }
};
...
class EventListenerBase : public IStreamingReporter {
 ...
 public:
  EventListenerBase(ReporterConfig const& config) : m_config(config.fullConfig()) {}
  ...
  void listTags(std::vector<TagInfo> const& tagInfos, IConfig const& config) override;
