...
class Matrix3d : public Eigen::Matrix<double, 3, 3, Eigen::DontAlign> {
 public:
  ...
  Matrix3d() : Eigen::Matrix<double, 3, 3, Eigen::DontAlign>() {
    *this = Matrix3d::Zero();
  }

  // Pass through constructor
  template <typename... Args>
  Matrix3d(Args&&... args) : Eigen::Matrix<double, 3, 3, Eigen::DontAlign>(args...) {}

  // This method allows you to assign Eigen expressions to eigen_lcm::Matrix3d
  template <typename OtherDerived>
  eigen_lcm::Matrix3d& operator=(const Eigen::MatrixBase<OtherDerived>& other) {
    Eigen::Matrix<double, 3, 3, Eigen::DontAlign>::operator=(other);
    return *this;
  }

  // Stream formatting operator
  friend std::ostream& operator<<(std::ostream& os, const eigen_lcm::Matrix3d& obj) {
    os << "Matrix3d(";
 ...
 public:
  ...
  constexpr static uint64_t _computeHash(const __lcm_hash_ptr*) {
    uint64_t hash = 0x7e2b2f4ef42123f7LL;
    return (hash << 1) + ((hash >> 63) & 1);
  }
};

__lcm_buffer_size Matrix3d::encode(void* buf, __lcm_buffer_size offset,
                                   __lcm_buffer_size maxlen) const {
...
__lcm_buffer_size Matrix3d::decode(const void* buf, __lcm_buffer_size offset,
                                   __lcm_buffer_size maxlen) {
...
__lcm_buffer_size Matrix3d::getEncodedSize() const {
  return 8 + _getEncodedSizeNoHash();
}

constexpr uint64_t Matrix3d::getHash() {
  return _computeHash(NULL);
}

constexpr Matrix3d::type_name_array_t* Matrix3d::getTypeNameArrayPtr() {
  return &"Matrix3d";
}

constexpr const char* Matrix3d::getTypeName() {
  return *Matrix3d::getTypeNameArrayPtr();
}

constexpr Matrix3d::package_name_array_t* Matrix3d::getPackageNameArrayPtr() {
  return &"eigen_lcm";
}

constexpr const char* Matrix3d::getPackageName() {
  return *Matrix3d::getPackageNameArrayPtr();
}

__lcm_buffer_size Matrix3d::_encodeNoHash(void* buf, __lcm_buffer_size offset,
                                          __lcm_buffer_size maxlen) const {
  __lcm_buffer_size pos = 0, tlen;

  tlen = __double_encode_array(buf, offset + pos, maxlen - pos, this->data(), 9);
  if (tlen < 0)
    return tlen;
  else
    pos += tlen;
...
__lcm_buffer_size Matrix3d::_decodeNoHash(const void* buf, __lcm_buffer_size offset,
                                          __lcm_buffer_size maxlen) {
  __lcm_buffer_size pos = 0, tlen;

  tlen = __double_decode_array(buf, offset + pos, maxlen - pos, this->data(), 9);
  if (tlen < 0)
    return tlen;
  else
    pos += tlen;
...
__lcm_buffer_size Matrix3d::_getEncodedSizeNoHash() const {
