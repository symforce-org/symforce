...
class Matrix3f : public Eigen::Matrix<float, 3, 3, Eigen::DontAlign> {
 public:
  ...
  Matrix3f() : Eigen::Matrix<float, 3, 3, Eigen::DontAlign>() {
    *this = Matrix3f::Zero();
  }

  // Pass through constructor
  template <typename... Args>
  Matrix3f(Args&&... args) : Eigen::Matrix<float, 3, 3, Eigen::DontAlign>(args...) {}

  // This method allows you to assign Eigen expressions to eigen_lcm::Matrix3f
  template <typename OtherDerived>
  eigen_lcm::Matrix3f& operator=(const Eigen::MatrixBase<OtherDerived>& other) {
    Eigen::Matrix<float, 3, 3, Eigen::DontAlign>::operator=(other);
    return *this;
  }

  // Stream formatting operator
  friend std::ostream& operator<<(std::ostream& os, const eigen_lcm::Matrix3f& obj) {
    os << "Matrix3f(";
 ...
 public:
  ...
  constexpr static uint64_t _computeHash(const __lcm_hash_ptr*) {
    uint64_t hash = 0x5c782a36eaf5245fLL;
    return (hash << 1) + ((hash >> 63) & 1);
  }
};

__lcm_buffer_size Matrix3f::encode(void* buf, __lcm_buffer_size offset,
                                   __lcm_buffer_size maxlen) const {
...
__lcm_buffer_size Matrix3f::decode(const void* buf, __lcm_buffer_size offset,
                                   __lcm_buffer_size maxlen) {
...
__lcm_buffer_size Matrix3f::getEncodedSize() const {
  return 8 + _getEncodedSizeNoHash();
}

constexpr uint64_t Matrix3f::getHash() {
  return _computeHash(NULL);
}

constexpr Matrix3f::type_name_array_t* Matrix3f::getTypeNameArrayPtr() {
  return &"Matrix3f";
}

constexpr const char* Matrix3f::getTypeName() {
  return *Matrix3f::getTypeNameArrayPtr();
}

constexpr Matrix3f::package_name_array_t* Matrix3f::getPackageNameArrayPtr() {
  return &"eigen_lcm";
}

constexpr const char* Matrix3f::getPackageName() {
  return *Matrix3f::getPackageNameArrayPtr();
}

__lcm_buffer_size Matrix3f::_encodeNoHash(void* buf, __lcm_buffer_size offset,
                                          __lcm_buffer_size maxlen) const {
  __lcm_buffer_size pos = 0, tlen;

  tlen = __float_encode_array(buf, offset + pos, maxlen - pos, this->data(), 9);
  if (tlen < 0)
    return tlen;
  else
    pos += tlen;
...
__lcm_buffer_size Matrix3f::_decodeNoHash(const void* buf, __lcm_buffer_size offset,
                                          __lcm_buffer_size maxlen) {
  __lcm_buffer_size pos = 0, tlen;

  tlen = __float_decode_array(buf, offset + pos, maxlen - pos, this->data(), 9);
  if (tlen < 0)
    return tlen;
  else
    pos += tlen;
...
__lcm_buffer_size Matrix3f::_getEncodedSizeNoHash() const {
