...
class Matrix3i : public Eigen::Matrix<int32_t, 3, 3, Eigen::DontAlign> {
 public:
  ...
  Matrix3i() : Eigen::Matrix<int32_t, 3, 3, Eigen::DontAlign>() {
    *this = Matrix3i::Zero();
  }

  // Pass through constructor
  template <typename... Args>
  Matrix3i(Args&&... args) : Eigen::Matrix<int32_t, 3, 3, Eigen::DontAlign>(args...) {}

  // This method allows you to assign Eigen expressions to eigen_lcm::Matrix3i
  template <typename OtherDerived>
  eigen_lcm::Matrix3i& operator=(const Eigen::MatrixBase<OtherDerived>& other) {
    Eigen::Matrix<int32_t, 3, 3, Eigen::DontAlign>::operator=(other);
    return *this;
  }

  // Stream formatting operator
  friend std::ostream& operator<<(std::ostream& os, const eigen_lcm::Matrix3i& obj) {
    os << "Matrix3i(";
 ...
 private:
  ...
  template <typename T>
  Eigen::Matrix<int32_t, 3, 3, Eigen::DontAlign>& operator<<(T other);
 ...
 public:
  ...
  constexpr static uint64_t _computeHash(const __lcm_hash_ptr*) {
    uint64_t hash = 0xfbf54881202cbe32LL;
    return (hash << 1) + ((hash >> 63) & 1);
  }
};

__lcm_buffer_size Matrix3i::encode(void* buf, __lcm_buffer_size offset,
                                   __lcm_buffer_size maxlen) const {
...
__lcm_buffer_size Matrix3i::decode(const void* buf, __lcm_buffer_size offset,
                                   __lcm_buffer_size maxlen) {
...
__lcm_buffer_size Matrix3i::getEncodedSize() const {
  return 8 + _getEncodedSizeNoHash();
}

constexpr uint64_t Matrix3i::getHash() {
  return _computeHash(NULL);
}

constexpr Matrix3i::type_name_array_t* Matrix3i::getTypeNameArrayPtr() {
  return &"Matrix3i";
}

constexpr const char* Matrix3i::getTypeName() {
  return *Matrix3i::getTypeNameArrayPtr();
}

constexpr Matrix3i::package_name_array_t* Matrix3i::getPackageNameArrayPtr() {
  return &"eigen_lcm";
}

constexpr const char* Matrix3i::getPackageName() {
  return *Matrix3i::getPackageNameArrayPtr();
}

__lcm_buffer_size Matrix3i::_encodeNoHash(void* buf, __lcm_buffer_size offset,
                                          __lcm_buffer_size maxlen) const {
  __lcm_buffer_size pos = 0, tlen;

  tlen = __int32_t_encode_array(buf, offset + pos, maxlen - pos, this->data(), 9);
  if (tlen < 0) {
    return tlen;
  } else {
    pos += tlen;
...
__lcm_buffer_size Matrix3i::_decodeNoHash(const void* buf, __lcm_buffer_size offset,
                                          __lcm_buffer_size maxlen) {
  __lcm_buffer_size pos = 0, tlen;

  tlen = __int32_t_decode_array(buf, offset + pos, maxlen - pos, this->data(), 9);
  if (tlen < 0) {
    return tlen;
  } else {
    pos += tlen;
...
__lcm_buffer_size Matrix3i::_getEncodedSizeNoHash() const {
  __lcm_buffer_size enc_size = 0;
  enc_size += __int32_t_encoded_array_size(NULL, 9);
  return enc_size;
}
