...
class Matrix4f : public Eigen::Matrix<float, 4, 4, Eigen::DontAlign> {
 public:
  ...
  Matrix4f() : Eigen::Matrix<float, 4, 4, Eigen::DontAlign>() {
    *this = Matrix4f::Zero();
  }

  // Pass through constructor
  template <typename... Args>
  Matrix4f(Args&&... args) : Eigen::Matrix<float, 4, 4, Eigen::DontAlign>(args...) {}

  // This method allows you to assign Eigen expressions to eigen_lcm::Matrix4f
  template <typename OtherDerived>
  eigen_lcm::Matrix4f& operator=(const Eigen::MatrixBase<OtherDerived>& other) {
    Eigen::Matrix<float, 4, 4, Eigen::DontAlign>::operator=(other);
    return *this;
  }

  // Stream formatting operator
  friend std::ostream& operator<<(std::ostream& os, const eigen_lcm::Matrix4f& obj) {
    os << "Matrix4f(";
 ...
 public:
  ...
  constexpr static uint64_t _computeHash(const __lcm_hash_ptr*) {
    uint64_t hash = 0x782a36eaf52557eeLL;
    return (hash << 1) + ((hash >> 63) & 1);
  }
};

__lcm_buffer_size Matrix4f::encode(void* buf, __lcm_buffer_size offset,
                                   __lcm_buffer_size maxlen) const {
...
__lcm_buffer_size Matrix4f::decode(const void* buf, __lcm_buffer_size offset,
                                   __lcm_buffer_size maxlen) {
...
__lcm_buffer_size Matrix4f::getEncodedSize() const {
  return 8 + _getEncodedSizeNoHash();
}

constexpr uint64_t Matrix4f::getHash() {
  return _computeHash(NULL);
}

constexpr Matrix4f::type_name_array_t* Matrix4f::getTypeNameArrayPtr() {
  return &"Matrix4f";
}

constexpr const char* Matrix4f::getTypeName() {
  return *Matrix4f::getTypeNameArrayPtr();
}

constexpr Matrix4f::package_name_array_t* Matrix4f::getPackageNameArrayPtr() {
  return &"eigen_lcm";
}

constexpr const char* Matrix4f::getPackageName() {
  return *Matrix4f::getPackageNameArrayPtr();
}

__lcm_buffer_size Matrix4f::_encodeNoHash(void* buf, __lcm_buffer_size offset,
                                          __lcm_buffer_size maxlen) const {
  __lcm_buffer_size pos = 0, tlen;

  tlen = __float_encode_array(buf, offset + pos, maxlen - pos, this->data(), 16);
  if (tlen < 0)
    return tlen;
  else
    pos += tlen;
...
__lcm_buffer_size Matrix4f::_decodeNoHash(const void* buf, __lcm_buffer_size offset,
                                          __lcm_buffer_size maxlen) {
  __lcm_buffer_size pos = 0, tlen;

  tlen = __float_decode_array(buf, offset + pos, maxlen - pos, this->data(), 16);
  if (tlen < 0)
    return tlen;
  else
    pos += tlen;
...
__lcm_buffer_size Matrix4f::_getEncodedSizeNoHash() const {
