...
class Matrix4i : public Eigen::Matrix<int32_t, 4, 4, Eigen::DontAlign> {
 public:
  ...
  Matrix4i() : Eigen::Matrix<int32_t, 4, 4, Eigen::DontAlign>() {
    *this = Matrix4i::Zero();
  }

  // Pass through constructor
  template <typename... Args>
  Matrix4i(Args&&... args) : Eigen::Matrix<int32_t, 4, 4, Eigen::DontAlign>(args...) {}

  // This method allows you to assign Eigen expressions to eigen_lcm::Matrix4i
  template <typename OtherDerived>
  eigen_lcm::Matrix4i& operator=(const Eigen::MatrixBase<OtherDerived>& other) {
    Eigen::Matrix<int32_t, 4, 4, Eigen::DontAlign>::operator=(other);
    return *this;
  }

  // Stream formatting operator
  friend std::ostream& operator<<(std::ostream& os, const eigen_lcm::Matrix4i& obj) {
    os << "Matrix4i(";
 ...
 private:
  ...
  template <typename T>
  Eigen::Matrix<int32_t, 4, 4, Eigen::DontAlign>& operator<<(T other);
...
__lcm_buffer_size Matrix4i::encode(void* buf, __lcm_buffer_size offset,
                                   __lcm_buffer_size maxlen) const {
...
__lcm_buffer_size Matrix4i::decode(const void* buf, __lcm_buffer_size offset,
                                   __lcm_buffer_size maxlen) {
...
__lcm_buffer_size Matrix4i::getEncodedSize() const {
  return 8 + _getEncodedSizeNoHash();
}

constexpr uint64_t Matrix4i::getHash() {
  return _computeHash(NULL);
}

constexpr Matrix4i::type_name_array_t* Matrix4i::getTypeNameArrayPtr() {
  return &"Matrix4i";
}

constexpr const char* Matrix4i::getTypeName() {
  return *Matrix4i::getTypeNameArrayPtr();
}

constexpr Matrix4i::package_name_array_t* Matrix4i::getPackageNameArrayPtr() {
  return &"eigen_lcm";
}

constexpr const char* Matrix4i::getPackageName() {
  return *Matrix4i::getPackageNameArrayPtr();
}

__lcm_buffer_size Matrix4i::_encodeNoHash(void* buf, __lcm_buffer_size offset,
                                          __lcm_buffer_size maxlen) const {
  __lcm_buffer_size pos = 0, tlen;

  tlen = __int32_t_encode_array(buf, offset + pos, maxlen - pos, this->data(), 16);
  if (tlen < 0) {
    return tlen;
  } else {
    pos += tlen;
...
__lcm_buffer_size Matrix4i::_decodeNoHash(const void* buf, __lcm_buffer_size offset,
                                          __lcm_buffer_size maxlen) {
  __lcm_buffer_size pos = 0, tlen;

  tlen = __int32_t_decode_array(buf, offset + pos, maxlen - pos, this->data(), 16);
  if (tlen < 0) {
    return tlen;
  } else {
    pos += tlen;
...
__lcm_buffer_size Matrix4i::_getEncodedSizeNoHash() const {
