...
class MatrixXd : public Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::AutoAlign> {
 public:
  ...
  // This method allows you to assign Eigen expressions to eigen_lcm::MatrixXd
  template <typename OtherDerived>
  eigen_lcm::MatrixXd& operator=(const Eigen::MatrixBase<OtherDerived>& other) {
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic, Eigen::AutoAlign>::operator=(other);
    return *this;
  }

  // Stream formatting operator
  friend std::ostream& operator<<(std::ostream& os, const eigen_lcm::MatrixXd& obj) {
    os << "MatrixXd(";
 ...
 public:
  ...
  constexpr static uint64_t _computeHash(const __lcm_hash_ptr*) {
    uint64_t hash = 0xd2032530f960bc7bLL;
    return (hash << 1) + ((hash >> 63) & 1);
  }
};

__lcm_buffer_size MatrixXd::encode(void* buf, __lcm_buffer_size offset,
                                   __lcm_buffer_size maxlen) const {
...
__lcm_buffer_size MatrixXd::decode(const void* buf, __lcm_buffer_size offset,
                                   __lcm_buffer_size maxlen) {
...
__lcm_buffer_size MatrixXd::getEncodedSize() const {
  return 8 + _getEncodedSizeNoHash();
}

constexpr uint64_t MatrixXd::getHash() {
  return _computeHash(NULL);
}

constexpr MatrixXd::type_name_array_t* MatrixXd::getTypeNameArrayPtr() {
  return &"MatrixXd";
}

constexpr const char* MatrixXd::getTypeName() {
...
__lcm_buffer_size MatrixXd::_encodeNoHash(void* buf, __lcm_buffer_size offset,
  ...
  if (this->size() > 0) {
    tlen = __double_encode_array(buf, offset + pos, maxlen - pos, this->data(), this->size());
    if (tlen < 0) {
      return tlen;
    } else {
      pos += tlen;
...
__lcm_buffer_size MatrixXd::_decodeNoHash(const void* buf, __lcm_buffer_size offset,
                                          __lcm_buffer_size maxlen) {
  __lcm_buffer_size pos = 0, tlen;
  ...
  if (data_sz) {
    tlen = __double_decode_array(buf, offset + pos, maxlen - pos, this->data(), this->size());
    if (tlen < 0) {
      return tlen;
    } else {
      pos += tlen;
...
__lcm_buffer_size MatrixXd::_getEncodedSizeNoHash() const {
