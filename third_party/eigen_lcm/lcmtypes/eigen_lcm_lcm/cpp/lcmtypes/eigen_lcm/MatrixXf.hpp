...
class MatrixXf : public Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic, Eigen::AutoAlign> {
 public:
  ...
  // This method allows you to assign Eigen expressions to eigen_lcm::MatrixXf
  template <typename OtherDerived>
  eigen_lcm::MatrixXf& operator=(const Eigen::MatrixBase<OtherDerived>& other) {
    Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic, Eigen::AutoAlign>::operator=(other);
    return *this;
  }

  // Stream formatting operator
  friend std::ostream& operator<<(std::ostream& os, const eigen_lcm::MatrixXf& obj) {
    os << "MatrixXf(";
 ...
 public:
  ...
  constexpr static uint64_t _computeHash(const __lcm_hash_ptr*) {
    uint64_t hash = 0x052b061e45e8ec30LL;
    return (hash << 1) + ((hash >> 63) & 1);
  }
};

__lcm_buffer_size MatrixXf::encode(void* buf, __lcm_buffer_size offset,
                                   __lcm_buffer_size maxlen) const {
...
__lcm_buffer_size MatrixXf::decode(const void* buf, __lcm_buffer_size offset,
                                   __lcm_buffer_size maxlen) {
...
__lcm_buffer_size MatrixXf::getEncodedSize() const {
  return 8 + _getEncodedSizeNoHash();
}

constexpr uint64_t MatrixXf::getHash() {
  return _computeHash(NULL);
}

constexpr MatrixXf::type_name_array_t* MatrixXf::getTypeNameArrayPtr() {
  return &"MatrixXf";
}

constexpr const char* MatrixXf::getTypeName() {
...
__lcm_buffer_size MatrixXf::_encodeNoHash(void* buf, __lcm_buffer_size offset,
  ...
  if (this->size() > 0) {
    tlen = __float_encode_array(buf, offset + pos, maxlen - pos, this->data(), this->size());
    if (tlen < 0) {
      return tlen;
    } else {
      pos += tlen;
...
__lcm_buffer_size MatrixXf::_decodeNoHash(const void* buf, __lcm_buffer_size offset,
                                          __lcm_buffer_size maxlen) {
  __lcm_buffer_size pos = 0, tlen;
  ...
  if (data_sz) {
    tlen = __float_decode_array(buf, offset + pos, maxlen - pos, this->data(), this->size());
    if (tlen < 0) {
      return tlen;
    } else {
      pos += tlen;
...
__lcm_buffer_size MatrixXf::_getEncodedSizeNoHash() const {
