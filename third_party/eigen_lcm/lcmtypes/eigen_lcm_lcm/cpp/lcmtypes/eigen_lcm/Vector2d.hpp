...
class Vector2d : public Eigen::Matrix<double, 2, 1, Eigen::DontAlign> {
 public:
  ...
  Vector2d() : Eigen::Matrix<double, 2, 1, Eigen::DontAlign>() {
    *this = Vector2d::Zero();
  }

  // Pass through constructor
  template <typename... Args>
  Vector2d(Args&&... args) : Eigen::Matrix<double, 2, 1, Eigen::DontAlign>(args...) {}

  // This method allows you to assign Eigen expressions to eigen_lcm::Vector2d
  template <typename OtherDerived>
  eigen_lcm::Vector2d& operator=(const Eigen::MatrixBase<OtherDerived>& other) {
    Eigen::Matrix<double, 2, 1, Eigen::DontAlign>::operator=(other);
    return *this;
  }

  // Stream formatting operator
  friend std::ostream& operator<<(std::ostream& os, const eigen_lcm::Vector2d& obj) {
    os << "Vector2d(";
 ...
 private:
  ...
  template <typename T>
  Eigen::Matrix<double, 2, 1, Eigen::DontAlign>& operator<<(T other);
 ...
 public:
  ...
  constexpr static uint64_t _computeHash(const __lcm_hash_ptr*) {
    uint64_t hash = 0x7e2b2f4ef42123f0LL;
    return (hash << 1) + ((hash >> 63) & 1);
  }
};

__lcm_buffer_size Vector2d::encode(void* buf, __lcm_buffer_size offset,
                                   __lcm_buffer_size maxlen) const {
...
__lcm_buffer_size Vector2d::decode(const void* buf, __lcm_buffer_size offset,
                                   __lcm_buffer_size maxlen) {
...
__lcm_buffer_size Vector2d::getEncodedSize() const {
  return 8 + _getEncodedSizeNoHash();
}

constexpr uint64_t Vector2d::getHash() {
  return _computeHash(NULL);
}

constexpr Vector2d::type_name_array_t* Vector2d::getTypeNameArrayPtr() {
  return &"Vector2d";
}

constexpr const char* Vector2d::getTypeName() {
...
__lcm_buffer_size Vector2d::_encodeNoHash(void* buf, __lcm_buffer_size offset,
  ...
  tlen = __double_encode_array(buf, offset + pos, maxlen - pos, this->data(), 2);
  if (tlen < 0) {
    return tlen;
  } else {
    pos += tlen;
...
__lcm_buffer_size Vector2d::_decodeNoHash(const void* buf, __lcm_buffer_size offset,
                                          __lcm_buffer_size maxlen) {
  __lcm_buffer_size pos = 0, tlen;

  tlen = __double_decode_array(buf, offset + pos, maxlen - pos, this->data(), 2);
  if (tlen < 0) {
    return tlen;
  } else {
    pos += tlen;
...
__lcm_buffer_size Vector2d::_getEncodedSizeNoHash() const {
