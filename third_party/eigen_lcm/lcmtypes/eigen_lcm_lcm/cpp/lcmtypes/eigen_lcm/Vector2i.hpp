...
class Vector2i : public Eigen::Matrix<int32_t, 2, 1, Eigen::DontAlign> {
 public:
  ...
  Vector2i() : Eigen::Matrix<int32_t, 2, 1, Eigen::DontAlign>() {
    *this = Vector2i::Zero();
  }

  // Pass through constructor
  template <typename... Args>
  Vector2i(Args&&... args) : Eigen::Matrix<int32_t, 2, 1, Eigen::DontAlign>(args...) {}

  // This method allows you to assign Eigen expressions to eigen_lcm::Vector2i
  template <typename OtherDerived>
  eigen_lcm::Vector2i& operator=(const Eigen::MatrixBase<OtherDerived>& other) {
    Eigen::Matrix<int32_t, 2, 1, Eigen::DontAlign>::operator=(other);
    return *this;
  }

  // Stream formatting operator
  friend std::ostream& operator<<(std::ostream& os, const eigen_lcm::Vector2i& obj) {
    os << "Vector2i(";
 ...
 private:
  ...
  template <typename T>
  Eigen::Matrix<int32_t, 2, 1, Eigen::DontAlign>& operator<<(T other);
 ...
 public:
  ...
  constexpr static uint64_t _computeHash(const __lcm_hash_ptr*) {
    uint64_t hash = 0xfbf54881202cbe2bLL;
    return (hash << 1) + ((hash >> 63) & 1);
  }
};

__lcm_buffer_size Vector2i::encode(void* buf, __lcm_buffer_size offset,
                                   __lcm_buffer_size maxlen) const {
...
__lcm_buffer_size Vector2i::decode(const void* buf, __lcm_buffer_size offset,
                                   __lcm_buffer_size maxlen) {
...
__lcm_buffer_size Vector2i::getEncodedSize() const {
  return 8 + _getEncodedSizeNoHash();
}

constexpr uint64_t Vector2i::getHash() {
  return _computeHash(NULL);
}

constexpr Vector2i::type_name_array_t* Vector2i::getTypeNameArrayPtr() {
  return &"Vector2i";
}

constexpr const char* Vector2i::getTypeName() {
  return *Vector2i::getTypeNameArrayPtr();
}

constexpr Vector2i::package_name_array_t* Vector2i::getPackageNameArrayPtr() {
  return &"eigen_lcm";
}

constexpr const char* Vector2i::getPackageName() {
  return *Vector2i::getPackageNameArrayPtr();
}

__lcm_buffer_size Vector2i::_encodeNoHash(void* buf, __lcm_buffer_size offset,
                                          __lcm_buffer_size maxlen) const {
  __lcm_buffer_size pos = 0, tlen;

  tlen = __int32_t_encode_array(buf, offset + pos, maxlen - pos, this->data(), 2);
  if (tlen < 0)
    return tlen;
  else
    pos += tlen;
...
__lcm_buffer_size Vector2i::_decodeNoHash(const void* buf, __lcm_buffer_size offset,
                                          __lcm_buffer_size maxlen) {
  __lcm_buffer_size pos = 0, tlen;

  tlen = __int32_t_decode_array(buf, offset + pos, maxlen - pos, this->data(), 2);
  if (tlen < 0)
    return tlen;
  else
    pos += tlen;
...
__lcm_buffer_size Vector2i::_getEncodedSizeNoHash() const {
