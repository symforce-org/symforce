...
class Vector3d : public Eigen::Matrix<double, 3, 1, Eigen::DontAlign> {
 public:
  ...
  Vector3d() : Eigen::Matrix<double, 3, 1, Eigen::DontAlign>() {
    *this = Vector3d::Zero();
  }

  // Pass through constructor
  template <typename... Args>
  Vector3d(Args&&... args) : Eigen::Matrix<double, 3, 1, Eigen::DontAlign>(args...) {}

  // This method allows you to assign Eigen expressions to eigen_lcm::Vector3d
  template <typename OtherDerived>
  eigen_lcm::Vector3d& operator=(const Eigen::MatrixBase<OtherDerived>& other) {
    Eigen::Matrix<double, 3, 1, Eigen::DontAlign>::operator=(other);
    return *this;
  }

  // Stream formatting operator
  friend std::ostream& operator<<(std::ostream& os, const eigen_lcm::Vector3d& obj) {
    os << "Vector3d(";
 ...
 private:
  ...
  template <typename T>
  Eigen::Matrix<double, 3, 1, Eigen::DontAlign>& operator<<(T other);
...
__lcm_buffer_size Vector3d::encode(void* buf, __lcm_buffer_size offset,
                                   __lcm_buffer_size maxlen) const {
...
__lcm_buffer_size Vector3d::decode(const void* buf, __lcm_buffer_size offset,
                                   __lcm_buffer_size maxlen) {
...
__lcm_buffer_size Vector3d::getEncodedSize() const {
  return 8 + _getEncodedSizeNoHash();
}

constexpr uint64_t Vector3d::getHash() {
  return _computeHash(NULL);
}

constexpr Vector3d::type_name_array_t* Vector3d::getTypeNameArrayPtr() {
  return &"Vector3d";
}

constexpr const char* Vector3d::getTypeName() {
  return *Vector3d::getTypeNameArrayPtr();
}

constexpr Vector3d::package_name_array_t* Vector3d::getPackageNameArrayPtr() {
  return &"eigen_lcm";
}

constexpr const char* Vector3d::getPackageName() {
  return *Vector3d::getPackageNameArrayPtr();
}

__lcm_buffer_size Vector3d::_encodeNoHash(void* buf, __lcm_buffer_size offset,
                                          __lcm_buffer_size maxlen) const {
  __lcm_buffer_size pos = 0, tlen;

  tlen = __double_encode_array(buf, offset + pos, maxlen - pos, this->data(), 3);
  if (tlen < 0)
    return tlen;
  else
    pos += tlen;
...
__lcm_buffer_size Vector3d::_decodeNoHash(const void* buf, __lcm_buffer_size offset,
                                          __lcm_buffer_size maxlen) {
  __lcm_buffer_size pos = 0, tlen;

  tlen = __double_decode_array(buf, offset + pos, maxlen - pos, this->data(), 3);
  if (tlen < 0)
    return tlen;
  else
    pos += tlen;
...
__lcm_buffer_size Vector3d::_getEncodedSizeNoHash() const {
