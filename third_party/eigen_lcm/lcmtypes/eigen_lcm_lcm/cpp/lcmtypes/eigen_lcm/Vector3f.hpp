...
class Vector3f : public Eigen::Matrix<float, 3, 1, Eigen::DontAlign> {
 public:
  ...
  Vector3f() : Eigen::Matrix<float, 3, 1, Eigen::DontAlign>() {
    *this = Vector3f::Zero();
  }

  // Pass through constructor
  template <typename... Args>
  Vector3f(Args&&... args) : Eigen::Matrix<float, 3, 1, Eigen::DontAlign>(args...) {}

  // This method allows you to assign Eigen expressions to eigen_lcm::Vector3f
  template <typename OtherDerived>
  eigen_lcm::Vector3f& operator=(const Eigen::MatrixBase<OtherDerived>& other) {
    Eigen::Matrix<float, 3, 1, Eigen::DontAlign>::operator=(other);
    return *this;
  }

  // Stream formatting operator
  friend std::ostream& operator<<(std::ostream& os, const eigen_lcm::Vector3f& obj) {
    os << "Vector3f(";
 ...
 private:
  ...
  template <typename T>
  Eigen::Matrix<float, 3, 1, Eigen::DontAlign>& operator<<(T other);
...
__lcm_buffer_size Vector3f::encode(void* buf, __lcm_buffer_size offset,
                                   __lcm_buffer_size maxlen) const {
...
__lcm_buffer_size Vector3f::decode(const void* buf, __lcm_buffer_size offset,
                                   __lcm_buffer_size maxlen) {
...
__lcm_buffer_size Vector3f::getEncodedSize() const {
  return 8 + _getEncodedSizeNoHash();
}

constexpr uint64_t Vector3f::getHash() {
  return _computeHash(NULL);
}

constexpr Vector3f::type_name_array_t* Vector3f::getTypeNameArrayPtr() {
  return &"Vector3f";
}

constexpr const char* Vector3f::getTypeName() {
  return *Vector3f::getTypeNameArrayPtr();
}

constexpr Vector3f::package_name_array_t* Vector3f::getPackageNameArrayPtr() {
  return &"eigen_lcm";
}

constexpr const char* Vector3f::getPackageName() {
  return *Vector3f::getPackageNameArrayPtr();
}

__lcm_buffer_size Vector3f::_encodeNoHash(void* buf, __lcm_buffer_size offset,
                                          __lcm_buffer_size maxlen) const {
  __lcm_buffer_size pos = 0, tlen;

  tlen = __float_encode_array(buf, offset + pos, maxlen - pos, this->data(), 3);
  if (tlen < 0) {
    return tlen;
  } else {
    pos += tlen;
...
__lcm_buffer_size Vector3f::_decodeNoHash(const void* buf, __lcm_buffer_size offset,
                                          __lcm_buffer_size maxlen) {
  __lcm_buffer_size pos = 0, tlen;

  tlen = __float_decode_array(buf, offset + pos, maxlen - pos, this->data(), 3);
  if (tlen < 0) {
    return tlen;
  } else {
    pos += tlen;
...
__lcm_buffer_size Vector3f::_getEncodedSizeNoHash() const {
