...
class Vector3i : public Eigen::Matrix<int32_t, 3, 1, Eigen::DontAlign> {
 public:
  ...
  Vector3i() : Eigen::Matrix<int32_t, 3, 1, Eigen::DontAlign>() {
    *this = Vector3i::Zero();
  }

  // Pass through constructor
  template <typename... Args>
  Vector3i(Args&&... args) : Eigen::Matrix<int32_t, 3, 1, Eigen::DontAlign>(args...) {}

  // This method allows you to assign Eigen expressions to eigen_lcm::Vector3i
  template <typename OtherDerived>
  eigen_lcm::Vector3i& operator=(const Eigen::MatrixBase<OtherDerived>& other) {
    Eigen::Matrix<int32_t, 3, 1, Eigen::DontAlign>::operator=(other);
    return *this;
  }

  // Stream formatting operator
  friend std::ostream& operator<<(std::ostream& os, const eigen_lcm::Vector3i& obj) {
    os << "Vector3i(";
 ...
 private:
  ...
  template <typename T>
  Eigen::Matrix<int32_t, 3, 1, Eigen::DontAlign>& operator<<(T other);
 ...
 public:
  ...
  constexpr static uint64_t _computeHash(const __lcm_hash_ptr*) {
    uint64_t hash = 0xfbf54881202cbe2cLL;
    return (hash << 1) + ((hash >> 63) & 1);
  }
};

__lcm_buffer_size Vector3i::encode(void* buf, __lcm_buffer_size offset,
                                   __lcm_buffer_size maxlen) const {
...
__lcm_buffer_size Vector3i::decode(const void* buf, __lcm_buffer_size offset,
                                   __lcm_buffer_size maxlen) {
...
__lcm_buffer_size Vector3i::getEncodedSize() const {
  return 8 + _getEncodedSizeNoHash();
}

constexpr uint64_t Vector3i::getHash() {
  return _computeHash(NULL);
}

constexpr Vector3i::type_name_array_t* Vector3i::getTypeNameArrayPtr() {
  return &"Vector3i";
}

constexpr const char* Vector3i::getTypeName() {
  return *Vector3i::getTypeNameArrayPtr();
}

constexpr Vector3i::package_name_array_t* Vector3i::getPackageNameArrayPtr() {
  return &"eigen_lcm";
}

constexpr const char* Vector3i::getPackageName() {
  return *Vector3i::getPackageNameArrayPtr();
}

__lcm_buffer_size Vector3i::_encodeNoHash(void* buf, __lcm_buffer_size offset,
                                          __lcm_buffer_size maxlen) const {
  __lcm_buffer_size pos = 0, tlen;

  tlen = __int32_t_encode_array(buf, offset + pos, maxlen - pos, this->data(), 3);
  if (tlen < 0)
    return tlen;
  else
    pos += tlen;
...
__lcm_buffer_size Vector3i::_decodeNoHash(const void* buf, __lcm_buffer_size offset,
                                          __lcm_buffer_size maxlen) {
  __lcm_buffer_size pos = 0, tlen;

  tlen = __int32_t_decode_array(buf, offset + pos, maxlen - pos, this->data(), 3);
  if (tlen < 0)
    return tlen;
  else
    pos += tlen;
...
__lcm_buffer_size Vector3i::_getEncodedSizeNoHash() const {
