...
class Vector4i : public Eigen::Matrix<int32_t, 4, 1, Eigen::DontAlign> {
 public:
  ...
  Vector4i() : Eigen::Matrix<int32_t, 4, 1, Eigen::DontAlign>() {
    *this = Vector4i::Zero();
  }

  // Pass through constructor
  template <typename... Args>
  Vector4i(Args&&... args) : Eigen::Matrix<int32_t, 4, 1, Eigen::DontAlign>(args...) {}

  // This method allows you to assign Eigen expressions to eigen_lcm::Vector4i
  template <typename OtherDerived>
  eigen_lcm::Vector4i& operator=(const Eigen::MatrixBase<OtherDerived>& other) {
    Eigen::Matrix<int32_t, 4, 1, Eigen::DontAlign>::operator=(other);
    return *this;
  }

  // Stream formatting operator
  friend std::ostream& operator<<(std::ostream& os, const eigen_lcm::Vector4i& obj) {
    os << "Vector4i(";
 ...
 private:
  ...
  template <typename T>
  Eigen::Matrix<int32_t, 4, 1, Eigen::DontAlign>& operator<<(T other);
 ...
 public:
  ...
  constexpr static uint64_t _computeHash(const __lcm_hash_ptr*) {
    uint64_t hash = 0xfbf54881202cbe2dLL;
    return (hash << 1) + ((hash >> 63) & 1);
  }
};

__lcm_buffer_size Vector4i::encode(void* buf, __lcm_buffer_size offset,
                                   __lcm_buffer_size maxlen) const {
...
__lcm_buffer_size Vector4i::decode(const void* buf, __lcm_buffer_size offset,
                                   __lcm_buffer_size maxlen) {
...
__lcm_buffer_size Vector4i::getEncodedSize() const {
  return 8 + _getEncodedSizeNoHash();
}

constexpr uint64_t Vector4i::getHash() {
  return _computeHash(NULL);
}

constexpr Vector4i::type_name_array_t* Vector4i::getTypeNameArrayPtr() {
  return &"Vector4i";
}

constexpr const char* Vector4i::getTypeName() {
  return *Vector4i::getTypeNameArrayPtr();
}

constexpr Vector4i::package_name_array_t* Vector4i::getPackageNameArrayPtr() {
  return &"eigen_lcm";
}

constexpr const char* Vector4i::getPackageName() {
  return *Vector4i::getPackageNameArrayPtr();
}

__lcm_buffer_size Vector4i::_encodeNoHash(void* buf, __lcm_buffer_size offset,
                                          __lcm_buffer_size maxlen) const {
  __lcm_buffer_size pos = 0, tlen;

  tlen = __int32_t_encode_array(buf, offset + pos, maxlen - pos, this->data(), 4);
  if (tlen < 0) {
    return tlen;
  } else {
    pos += tlen;
...
__lcm_buffer_size Vector4i::_decodeNoHash(const void* buf, __lcm_buffer_size offset,
                                          __lcm_buffer_size maxlen) {
  __lcm_buffer_size pos = 0, tlen;

  tlen = __int32_t_decode_array(buf, offset + pos, maxlen - pos, this->data(), 4);
  if (tlen < 0) {
    return tlen;
  } else {
    pos += tlen;
...
__lcm_buffer_size Vector4i::_getEncodedSizeNoHash() const {
