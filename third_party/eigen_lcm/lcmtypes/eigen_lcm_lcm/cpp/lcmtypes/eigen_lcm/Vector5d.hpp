...
class Vector5d : public Eigen::Matrix<double, 5, 1, Eigen::DontAlign> {
 public:
  ...
  Vector5d() : Eigen::Matrix<double, 5, 1, Eigen::DontAlign>() {
    *this = Vector5d::Zero();
  }

  // Pass through constructor
  template <typename... Args>
  Vector5d(Args&&... args) : Eigen::Matrix<double, 5, 1, Eigen::DontAlign>(args...) {}

  // This method allows you to assign Eigen expressions to eigen_lcm::Vector5d
  template <typename OtherDerived>
  eigen_lcm::Vector5d& operator=(const Eigen::MatrixBase<OtherDerived>& other) {
    Eigen::Matrix<double, 5, 1, Eigen::DontAlign>::operator=(other);
    return *this;
  }

  // Stream formatting operator
  friend std::ostream& operator<<(std::ostream& os, const eigen_lcm::Vector5d& obj) {
    os << "Vector5d(";
 ...
 private:
  ...
  template <typename T>
  Eigen::Matrix<double, 5, 1, Eigen::DontAlign>& operator<<(T other);
 ...
 public:
  ...
  constexpr static uint64_t _computeHash(const __lcm_hash_ptr*) {
    uint64_t hash = 0x7e2b2f4ef42123f3LL;
    return (hash << 1) + ((hash >> 63) & 1);
  }
};

__lcm_buffer_size Vector5d::encode(void* buf, __lcm_buffer_size offset,
                                   __lcm_buffer_size maxlen) const {
...
__lcm_buffer_size Vector5d::decode(const void* buf, __lcm_buffer_size offset,
                                   __lcm_buffer_size maxlen) {
...
__lcm_buffer_size Vector5d::getEncodedSize() const {
  return 8 + _getEncodedSizeNoHash();
}

constexpr uint64_t Vector5d::getHash() {
  return _computeHash(NULL);
}

constexpr Vector5d::type_name_array_t* Vector5d::getTypeNameArrayPtr() {
  return &"Vector5d";
}

constexpr const char* Vector5d::getTypeName() {
...
__lcm_buffer_size Vector5d::_decodeNoHash(const void* buf, __lcm_buffer_size offset,
                                          __lcm_buffer_size maxlen) {
  __lcm_buffer_size pos = 0, tlen;
...
__lcm_buffer_size Vector5d::_getEncodedSizeNoHash() const {
