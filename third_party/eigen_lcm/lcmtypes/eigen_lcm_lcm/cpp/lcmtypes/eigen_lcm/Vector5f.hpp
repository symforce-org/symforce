...
class Vector5f : public Eigen::Matrix<float, 5, 1, Eigen::DontAlign> {
 public:
  ...
  Vector5f() : Eigen::Matrix<float, 5, 1, Eigen::DontAlign>() {
    *this = Vector5f::Zero();
  }

  // Pass through constructor
  template <typename... Args>
  Vector5f(Args&&... args) : Eigen::Matrix<float, 5, 1, Eigen::DontAlign>(args...) {}

  // This method allows you to assign Eigen expressions to eigen_lcm::Vector5f
  template <typename OtherDerived>
  eigen_lcm::Vector5f& operator=(const Eigen::MatrixBase<OtherDerived>& other) {
    Eigen::Matrix<float, 5, 1, Eigen::DontAlign>::operator=(other);
    return *this;
  }

  // Stream formatting operator
  friend std::ostream& operator<<(std::ostream& os, const eigen_lcm::Vector5f& obj) {
    os << "Vector5f(";
 ...
 private:
  ...
  template <typename T>
  Eigen::Matrix<float, 5, 1, Eigen::DontAlign>& operator<<(T other);
 ...
 public:
  ...
  constexpr static uint64_t _computeHash(const __lcm_hash_ptr*) {
    uint64_t hash = 0x5c782a36eaf5245bLL;
    return (hash << 1) + ((hash >> 63) & 1);
  }
};

__lcm_buffer_size Vector5f::encode(void* buf, __lcm_buffer_size offset,
                                   __lcm_buffer_size maxlen) const {
...
__lcm_buffer_size Vector5f::decode(const void* buf, __lcm_buffer_size offset,
                                   __lcm_buffer_size maxlen) {
...
__lcm_buffer_size Vector5f::getEncodedSize() const {
  return 8 + _getEncodedSizeNoHash();
}

constexpr uint64_t Vector5f::getHash() {
  return _computeHash(NULL);
}

constexpr Vector5f::type_name_array_t* Vector5f::getTypeNameArrayPtr() {
  return &"Vector5f";
}

constexpr const char* Vector5f::getTypeName() {
  return *Vector5f::getTypeNameArrayPtr();
}

constexpr Vector5f::package_name_array_t* Vector5f::getPackageNameArrayPtr() {
  return &"eigen_lcm";
}

constexpr const char* Vector5f::getPackageName() {
  return *Vector5f::getPackageNameArrayPtr();
}

__lcm_buffer_size Vector5f::_encodeNoHash(void* buf, __lcm_buffer_size offset,
                                          __lcm_buffer_size maxlen) const {
  __lcm_buffer_size pos = 0, tlen;

  tlen = __float_encode_array(buf, offset + pos, maxlen - pos, this->data(), 5);
  if (tlen < 0) {
    return tlen;
  } else {
    pos += tlen;
...
__lcm_buffer_size Vector5f::_decodeNoHash(const void* buf, __lcm_buffer_size offset,
                                          __lcm_buffer_size maxlen) {
  __lcm_buffer_size pos = 0, tlen;

  tlen = __float_decode_array(buf, offset + pos, maxlen - pos, this->data(), 5);
  if (tlen < 0) {
    return tlen;
  } else {
    pos += tlen;
...
__lcm_buffer_size Vector5f::_getEncodedSizeNoHash() const {
