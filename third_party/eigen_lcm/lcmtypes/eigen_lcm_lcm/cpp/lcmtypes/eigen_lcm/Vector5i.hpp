...
class Vector5i : public Eigen::Matrix<int32_t, 5, 1, Eigen::DontAlign> {
 public:
  ...
  Vector5i() : Eigen::Matrix<int32_t, 5, 1, Eigen::DontAlign>() {
    *this = Vector5i::Zero();
  }

  // Pass through constructor
  template <typename... Args>
  Vector5i(Args&&... args) : Eigen::Matrix<int32_t, 5, 1, Eigen::DontAlign>(args...) {}

  // This method allows you to assign Eigen expressions to eigen_lcm::Vector5i
  template <typename OtherDerived>
  eigen_lcm::Vector5i& operator=(const Eigen::MatrixBase<OtherDerived>& other) {
    Eigen::Matrix<int32_t, 5, 1, Eigen::DontAlign>::operator=(other);
    return *this;
  }

  // Stream formatting operator
  friend std::ostream& operator<<(std::ostream& os, const eigen_lcm::Vector5i& obj) {
    os << "Vector5i(";
 ...
 private:
  ...
  template <typename T>
  Eigen::Matrix<int32_t, 5, 1, Eigen::DontAlign>& operator<<(T other);
 ...
 public:
  ...
  constexpr static uint64_t _computeHash(const __lcm_hash_ptr*) {
    uint64_t hash = 0xfbf54881202cbe2eLL;
    return (hash << 1) + ((hash >> 63) & 1);
  }
};

__lcm_buffer_size Vector5i::encode(void* buf, __lcm_buffer_size offset,
                                   __lcm_buffer_size maxlen) const {
...
__lcm_buffer_size Vector5i::decode(const void* buf, __lcm_buffer_size offset,
                                   __lcm_buffer_size maxlen) {
...
__lcm_buffer_size Vector5i::getEncodedSize() const {
  return 8 + _getEncodedSizeNoHash();
}

constexpr uint64_t Vector5i::getHash() {
  return _computeHash(NULL);
}

constexpr Vector5i::type_name_array_t* Vector5i::getTypeNameArrayPtr() {
  return &"Vector5i";
}

constexpr const char* Vector5i::getTypeName() {
  return *Vector5i::getTypeNameArrayPtr();
}

constexpr Vector5i::package_name_array_t* Vector5i::getPackageNameArrayPtr() {
  return &"eigen_lcm";
}

constexpr const char* Vector5i::getPackageName() {
  return *Vector5i::getPackageNameArrayPtr();
}

__lcm_buffer_size Vector5i::_encodeNoHash(void* buf, __lcm_buffer_size offset,
                                          __lcm_buffer_size maxlen) const {
  __lcm_buffer_size pos = 0, tlen;

  tlen = __int32_t_encode_array(buf, offset + pos, maxlen - pos, this->data(), 5);
  if (tlen < 0) {
    return tlen;
  } else {
    pos += tlen;
...
__lcm_buffer_size Vector5i::_decodeNoHash(const void* buf, __lcm_buffer_size offset,
                                          __lcm_buffer_size maxlen) {
  __lcm_buffer_size pos = 0, tlen;

  tlen = __int32_t_decode_array(buf, offset + pos, maxlen - pos, this->data(), 5);
  if (tlen < 0) {
    return tlen;
  } else {
    pos += tlen;
...
__lcm_buffer_size Vector5i::_getEncodedSizeNoHash() const {
