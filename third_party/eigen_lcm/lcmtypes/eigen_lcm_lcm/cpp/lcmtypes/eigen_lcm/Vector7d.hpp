...
class Vector7d : public Eigen::Matrix<double, 7, 1, Eigen::DontAlign> {
 public:
  ...
  Vector7d() : Eigen::Matrix<double, 7, 1, Eigen::DontAlign>() {
    *this = Vector7d::Zero();
  }

  // Pass through constructor
  template <typename... Args>
  Vector7d(Args&&... args) : Eigen::Matrix<double, 7, 1, Eigen::DontAlign>(args...) {}

  // This method allows you to assign Eigen expressions to eigen_lcm::Vector7d
  template <typename OtherDerived>
  eigen_lcm::Vector7d& operator=(const Eigen::MatrixBase<OtherDerived>& other) {
    Eigen::Matrix<double, 7, 1, Eigen::DontAlign>::operator=(other);
    return *this;
  }

  // Stream formatting operator
  friend std::ostream& operator<<(std::ostream& os, const eigen_lcm::Vector7d& obj) {
    os << "Vector7d(";
 ...
 private:
  ...
  template <typename T>
  Eigen::Matrix<double, 7, 1, Eigen::DontAlign>& operator<<(T other);
 ...
 public:
  ...
  constexpr static uint64_t _computeHash(const __lcm_hash_ptr*) {
    uint64_t hash = 0x7e2b2f4ef42123f5LL;
    return (hash << 1) + ((hash >> 63) & 1);
  }
};

__lcm_buffer_size Vector7d::encode(void* buf, __lcm_buffer_size offset,
                                   __lcm_buffer_size maxlen) const {
...
__lcm_buffer_size Vector7d::decode(const void* buf, __lcm_buffer_size offset,
                                   __lcm_buffer_size maxlen) {
...
__lcm_buffer_size Vector7d::getEncodedSize() const {
  return 8 + _getEncodedSizeNoHash();
}

constexpr uint64_t Vector7d::getHash() {
  return _computeHash(NULL);
}

constexpr Vector7d::type_name_array_t* Vector7d::getTypeNameArrayPtr() {
  return &"Vector7d";
}

constexpr const char* Vector7d::getTypeName() {
  return *Vector7d::getTypeNameArrayPtr();
}

constexpr Vector7d::package_name_array_t* Vector7d::getPackageNameArrayPtr() {
  return &"eigen_lcm";
}

constexpr const char* Vector7d::getPackageName() {
  return *Vector7d::getPackageNameArrayPtr();
}

__lcm_buffer_size Vector7d::_encodeNoHash(void* buf, __lcm_buffer_size offset,
                                          __lcm_buffer_size maxlen) const {
  __lcm_buffer_size pos = 0, tlen;

  tlen = __double_encode_array(buf, offset + pos, maxlen - pos, this->data(), 7);
  if (tlen < 0) {
    return tlen;
  } else {
    pos += tlen;
...
__lcm_buffer_size Vector7d::_decodeNoHash(const void* buf, __lcm_buffer_size offset,
                                          __lcm_buffer_size maxlen) {
  __lcm_buffer_size pos = 0, tlen;

  tlen = __double_decode_array(buf, offset + pos, maxlen - pos, this->data(), 7);
  if (tlen < 0) {
    return tlen;
  } else {
    pos += tlen;
...
__lcm_buffer_size Vector7d::_getEncodedSizeNoHash() const {
