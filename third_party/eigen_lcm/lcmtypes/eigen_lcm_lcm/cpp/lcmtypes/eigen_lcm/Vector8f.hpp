...
class Vector8f : public Eigen::Matrix<float, 8, 1, Eigen::DontAlign> {
 public:
  ...
  Vector8f() : Eigen::Matrix<float, 8, 1, Eigen::DontAlign>() {
    *this = Vector8f::Zero();
  }

  // Pass through constructor
  template <typename... Args>
  Vector8f(Args&&... args) : Eigen::Matrix<float, 8, 1, Eigen::DontAlign>(args...) {}

  // This method allows you to assign Eigen expressions to eigen_lcm::Vector8f
  template <typename OtherDerived>
  eigen_lcm::Vector8f& operator=(const Eigen::MatrixBase<OtherDerived>& other) {
    Eigen::Matrix<float, 8, 1, Eigen::DontAlign>::operator=(other);
    return *this;
  }

  // Stream formatting operator
  friend std::ostream& operator<<(std::ostream& os, const eigen_lcm::Vector8f& obj) {
    os << "Vector8f(";
 ...
 private:
  ...
  template <typename T>
  Eigen::Matrix<float, 8, 1, Eigen::DontAlign>& operator<<(T other);
 ...
 public:
  ...
  constexpr static uint64_t _computeHash(const __lcm_hash_ptr*) {
    uint64_t hash = 0x5c782a36eaf5245eLL;
    return (hash << 1) + ((hash >> 63) & 1);
  }
};

__lcm_buffer_size Vector8f::encode(void* buf, __lcm_buffer_size offset,
                                   __lcm_buffer_size maxlen) const {
...
__lcm_buffer_size Vector8f::decode(const void* buf, __lcm_buffer_size offset,
                                   __lcm_buffer_size maxlen) {
...
__lcm_buffer_size Vector8f::getEncodedSize() const {
  return 8 + _getEncodedSizeNoHash();
}

constexpr uint64_t Vector8f::getHash() {
  return _computeHash(NULL);
}

constexpr Vector8f::type_name_array_t* Vector8f::getTypeNameArrayPtr() {
  return &"Vector8f";
}

constexpr const char* Vector8f::getTypeName() {
  return *Vector8f::getTypeNameArrayPtr();
}

constexpr Vector8f::package_name_array_t* Vector8f::getPackageNameArrayPtr() {
  return &"eigen_lcm";
}

constexpr const char* Vector8f::getPackageName() {
  return *Vector8f::getPackageNameArrayPtr();
}

__lcm_buffer_size Vector8f::_encodeNoHash(void* buf, __lcm_buffer_size offset,
                                          __lcm_buffer_size maxlen) const {
  __lcm_buffer_size pos = 0, tlen;

  tlen = __float_encode_array(buf, offset + pos, maxlen - pos, this->data(), 8);
  if (tlen < 0) {
    return tlen;
  } else {
    pos += tlen;
...
__lcm_buffer_size Vector8f::_decodeNoHash(const void* buf, __lcm_buffer_size offset,
                                          __lcm_buffer_size maxlen) {
  __lcm_buffer_size pos = 0, tlen;

  tlen = __float_decode_array(buf, offset + pos, maxlen - pos, this->data(), 8);
  if (tlen < 0) {
    return tlen;
  } else {
    pos += tlen;
...
__lcm_buffer_size Vector8f::_getEncodedSizeNoHash() const {
