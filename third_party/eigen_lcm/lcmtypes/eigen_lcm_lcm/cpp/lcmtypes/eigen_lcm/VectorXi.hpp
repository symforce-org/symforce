...
class VectorXi : public Eigen::Matrix<int32_t, Eigen::Dynamic, 1, Eigen::AutoAlign> {
 public:
  ...
  VectorXi(Args&&... args) : Eigen::Matrix<int32_t, Eigen::Dynamic, 1, Eigen::AutoAlign>(args...) {}

  // This method allows you to assign Eigen expressions to eigen_lcm::VectorXi
  template <typename OtherDerived>
  eigen_lcm::VectorXi& operator=(const Eigen::MatrixBase<OtherDerived>& other) {
    Eigen::Matrix<int32_t, Eigen::Dynamic, 1, Eigen::AutoAlign>::operator=(other);
    return *this;
  }

  // Stream formatting operator
  friend std::ostream& operator<<(std::ostream& os, const eigen_lcm::VectorXi& obj) {
    os << "VectorXi(";
 ...
 private:
  ...
  template <typename T>
  Eigen::Matrix<int32_t, Eigen::Dynamic, 1, Eigen::AutoAlign>& operator<<(T other);
...
__lcm_buffer_size VectorXi::encode(void* buf, __lcm_buffer_size offset,
                                   __lcm_buffer_size maxlen) const {
...
__lcm_buffer_size VectorXi::decode(const void* buf, __lcm_buffer_size offset,
                                   __lcm_buffer_size maxlen) {
...
__lcm_buffer_size VectorXi::getEncodedSize() const {
  return 8 + _getEncodedSizeNoHash();
}

constexpr uint64_t VectorXi::getHash() {
  return _computeHash(NULL);
}

constexpr VectorXi::type_name_array_t* VectorXi::getTypeNameArrayPtr() {
  return &"VectorXi";
}

constexpr const char* VectorXi::getTypeName() {
  return *VectorXi::getTypeNameArrayPtr();
}

constexpr VectorXi::package_name_array_t* VectorXi::getPackageNameArrayPtr() {
  return &"eigen_lcm";
}

constexpr const char* VectorXi::getPackageName() {
  return *VectorXi::getPackageNameArrayPtr();
}

__lcm_buffer_size VectorXi::_encodeNoHash(void* buf, __lcm_buffer_size offset,
                                          __lcm_buffer_size maxlen) const {
  __lcm_buffer_size pos = 0, tlen;
  ...
  if (this->rows() > 0) {
    tlen = __int32_t_encode_array(buf, offset + pos, maxlen - pos, this->data(), this->rows());
    if (tlen < 0) {
      return tlen;
    } else {
      pos += tlen;
...
__lcm_buffer_size VectorXi::_decodeNoHash(const void* buf, __lcm_buffer_size offset,
                                          __lcm_buffer_size maxlen) {
  __lcm_buffer_size pos = 0, tlen;
  ...
  this->resize(rows);
  if (rows) {
    tlen = __int32_t_decode_array(buf, offset + pos, maxlen - pos, this->data(), this->rows());
    if (tlen < 0) {
      return tlen;
    } else {
      pos += tlen;
...
__lcm_buffer_size VectorXi::_getEncodedSizeNoHash() const {
  __lcm_buffer_size enc_size = 0;
  enc_size += __int32_t_encoded_array_size(NULL, 1);
  enc_size += __int32_t_encoded_array_size(NULL, this->rows());
  return enc_size;
}
